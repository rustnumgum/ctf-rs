...
pub struct Collective {
    pub ranks: usize,
    /// Source average peer-node count (`CommData::comm_nodes`), which may be
    /// fractional for an unreordered topology.
    pub nodes: f64,
    pub bytes: usize,
}
#[derive(Clone, Debug)]
pub enum Tree {
    ...
    Replicated {
        inputs: [Vec<Collective>; 2],
        output: Vec<Collective>,
        custom_reduce: bool,
        child: Box<Tree>,
    },
    Panels {
    ...
    pub fn estimate(&self, models: &Models, layers: usize) -> Estimate {
        ...
        match self {
            Self::Local {
                ...
                seconds: models.local_contraction(
                    *custom,
                    *folded,
                    operand_bytes.iter().sum::<usize>() as f64,
                    *flops,
                ),
                working_bytes: 0,
                internode_volume: 0.,
            ...
            Self::Virtual {
                ...
                let count = phases.iter().product::<usize>() as f64;
                let mut estimate = child.estimate(models, layers);
                estimate.seconds *= count;
                ...
                estimate.working_bytes += (orders.iter().sum::<usize>() + 4 * phases.len()) * 4;
                estimate
            }
            Self::Replicated {
                inputs,
                ...
                for comm in inputs.iter().flatten() {
                    estimate.seconds +=
                        models.communication(Communication::Broadcast, comm.ranks, comm.bytes);
                    estimate.internode_volume += comm.bytes as f64 * comm.nodes;
                }
                for comm in output {
                    estimate.seconds += models.communication(
                        Communication::Reduce {
                            custom: *custom_reduce,
                        },
                        comm.ranks,
                        comm.bytes,
                    );
            ...
            Self::Panels {
                ...
                assert!(*steps > 0);
                let calls = *steps as f64 / layers.min(*steps) as f64;
                let mut estimate = child.estimate(models, 1);
                let mut auxiliary = 0;
                ...
                estimate.seconds *= calls;
                estimate.internode_volume *= calls;
                estimate.working_bytes += panel_bytes.iter().sum::<usize>() + auxiliary;
                estimate
            }
        }
    }
