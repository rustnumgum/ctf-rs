...
pub fn relative_sign<T: PartialEq>(first: &[T], second: &[T]) -> i32 {
    ...
    for i in 0..first.len() {
        ...
        loop {
            let k = (0..first.len())
                .find(|&k| first[k] == second[j] && !seen[k])
                .expect("not a permutation");
            j = k;
            seen[j] = true;
            if j == i {
                break;
...
fn validate(indices: &[u8], sym: &[Symmetry]) {
    assert_eq!(indices.len(), sym.len());
    if let Some(last) = sym.last() {
        assert_eq!(*last, NS);
    }
}
...
pub fn align_triple(
    ...
    for (i, &label) in a.iter().enumerate() {
        let ib = b.iter().position(|&x| x == label);
        let ic = c.iter().position(|&x| x == label);
        if ib.is_some() || ic.is_some() {
            indices.push(Locator {
                order: 0,
                label,
                positions: [Some(i), ib, ic],
            });
        }
    }
    for (i, &label) in b.iter().enumerate() {
        if !a.contains(&label) {
            if let Some(ic) = c.iter().position(|&x| x == label) {
                indices.push(Locator {
                    order: 0,
                    label,
                    positions: [None, Some(i), Some(ic)],
                });
            }
        }
    }
    let symbols = [sym_a, sym_b, sym_c];
    let mut factor = 1;
    while !indices.is_empty() {
        let mut group = vec![indices.remove(0)];
        let mut i = 0;
        while i < indices.len() {
            let compatible = (0..3).all(|operand| {
                match (group[0].positions[operand], indices[i].positions[operand]) {
                    (None, None) => true,
                    (Some(x), Some(y)) => same_group(symbols[operand], x, y),
                    _ => false,
                }
            });
            if compatible {
                let mut entry = indices.remove(i);
                entry.order = group.len();
                group.push(entry);
            } else {
                i += 1;
            }
        ...
        for operand in if has_a { 1..3 } else { 2..3 } {
            if group[0].positions[operand].is_none() {
                continue;
            }
            ...
            for i in 0..group.len() {
                ...
                if operand == 1 {
                    b[destination] = group[i].label;
                } else {
                    c[destination] = group[i].label;
                }
            }
            if symbols[operand][group[0].positions[operand].unwrap()] == AS {
                factor *= relative_sign(&reference, &order);
            }
        }
    }
    factor
...
pub fn align_pair(a: &[u8], sym_a: &[Symmetry], b: &mut [u8], sym_b: &[Symmetry]) -> i32 {
    ...
    let mut indices: Vec<_> = a
        .iter()
        .enumerate()
        .filter_map(|(i, &label)| b.iter().position(|&x| x == label).map(|j| (0, label, i, j)))
        .collect();
    let mut factor = 1;
    while !indices.is_empty() {
        let mut group = vec![indices.remove(0)];
        let mut i = 0;
        while i < indices.len() {
            if same_group(sym_a, group[0].2, indices[i].2)
                && same_group(sym_b, group[0].3, indices[i].3)
            {
                let mut item = indices.remove(i);
                item.0 = group.len();
                group.push(item);
            } else {
                i += 1;
        ...
        for i in 0..group.len() {
            b[group[group[i].0].3] = group[i].1;
        }
        if sym_b[group[0].3] == AS {
            factor *= relative_sign(&order_a, &order_b);
        }
    }
    factor
}
