...
impl<T: Clone> Coo<T> {
    pub fn new(rows: usize, cols: usize, entries: Vec<(usize, usize, T)>) -> Self {
        assert!(entries.iter().all(|(r,c,_)| *r > 0 && *r <= rows && *c > 0 && *c <= cols));
    ...
    pub fn to_csr(&self) -> Csr<T> {
        ...
        ia[0] = 1;
        for &(r,_,_) in &self.entries { ia[r] += 1; }
        for r in 0..self.rows { ia[r+1] += ia[r]; }
        Csr { rows: self.rows, cols: self.cols, ia,
    ...
    pub fn to_ccsr(&self) -> Ccsr<T> {
        let mut out = Ccsr { rows: self.rows, cols: self.cols, row_encoding: Vec::new(),
            ia: vec![1], ja: Vec::new(), values: Vec::new() };
        for i in self.order() {
...
impl<T: Clone> Csr<T> {
    ...
    pub fn partition(&self, count: usize) -> Vec<Self> {
        assert!(count > 0);
        (0..count).map(|p| {
            let mut part = Self::empty(self.rows/count + usize::from(p < self.rows%count), self.cols);
            for r in (p..self.rows).step_by(count) { part.append_row(self, r); }
            part
        }).collect()
    }
    pub fn assemble(parts: &[Self]) -> Self {
        ...
        for (p,part) in parts.iter().enumerate() {
            ...
            assert_eq!(part.rows, rows/parts.len() + usize::from(p < rows%parts.len()));
        }
        for r in 0..rows { out.append_row(&parts[r%parts.len()], r/parts.len()); }
        out
    }
    ...
    pub fn add<A: Monoid<Element=T>>(&self, b: &Self, algebra: &A) -> Self {
        ...
        for r in 0..self.rows {
            present.fill(false);
            for matrix in [self,b] { for i in matrix.row(r) { present[matrix.ja[i]-1] = true; } }
            for c in 0..cols {
                if present[c] {
            ...
            for i in b.row(r) {
                let c = b.ja[i]-1;
                let at = reverse[c];
                out.values[at] = if present[c] { algebra.add(&out.values[at], &b.values[i]) }
                    else { b.values[i].clone() };
            }
            out.ia.push(out.values.len()+1);
        }
        out
    ...
    pub fn multiply_dense<A: Semiring<Element=T>>(&self, n: usize, b: &[T],
        alpha: &T, beta: &T, c: &mut [T], algebra: &A) {
        ...
        for r in 0..self.rows {
            for col in 0..n {
                ...
                if let Some(first) = indices.next() {
                    let mut sum = algebra.multiply(&self.values[first], &b[col*self.cols+self.ja[first]-1]);
                    for i in indices {
                        sum = algebra.add(&sum, &algebra.multiply(&self.values[i], &b[col*self.cols+self.ja[i]-1]));
                    }
    ...
    pub fn multiply_sparse<A: Semiring<Element=T>>(&self, b: &Self, alpha: &T,
        beta: &T, c: Option<&Self>, algebra: &A) -> Self where T: PartialEq {
        assert_eq!(self.cols, b.rows);
        if let Some(c) = c { assert_eq!(c.shape(), (self.rows,b.cols)); }
        let mut out = Self::empty(self.rows, b.cols);
        let mut present = vec![false; b.cols];
        for r in 0..self.rows {
            present.fill(false);
            for i in self.row(r) {
                for j in b.row(self.ja[i]-1) { present[b.ja[j]-1] = true; }
            }
            out.ia.push(out.ia[r] + present.iter().filter(|&&x| x).count());
        }
        ...
        for r in 0..self.rows {
            ...
            for i in self.row(r) {
                for j in b.row(self.ja[i]-1) {
                    let col = b.ja[j]-1;
                    present[col] = true;
                    accum[col] = algebra.add(&accum[col], &algebra.multiply(&self.values[i], &b.values[j]));
                }
            }
        ...
        if *beta != algebra.zero() {
            if let Some(c) = c {
                if !c.values.is_empty() {
                    ...
                    for v in &mut scaled.values { *v = algebra.multiply(beta, v); }
...
impl<T: Clone> Ccsr<T> {
    pub fn shape(&self) -> (usize,usize) { (self.rows,self.cols) }
    pub fn row_encoding(&self) -> &[usize] { &self.row_encoding }
    pub fn row_offsets(&self) -> &[usize] { &self.ia }
    pub fn columns(&self) -> &[usize] { &self.ja }
    pub fn values(&self) -> &[T] { &self.values }
    pub fn values_mut(&mut self) -> &mut [T] { &mut self.values }
    fn row(&self, r: usize) -> std::ops::Range<usize> { self.ia[r]-1..self.ia[r+1]-1 }
    fn empty(rows: usize, cols: usize) -> Self {
        Self { rows, cols, row_encoding: Vec::new(), ia: vec![1], ja: Vec::new(), values: Vec::new() }
    }
    ...
    pub fn to_coo(&self) -> Coo<T> {
        ...
        for (r,&encoding) in self.row_encoding.iter().enumerate() {
            for i in self.row(r) { entries.push((encoding,self.ja[i],self.values[i].clone())); }
        }
        Coo { rows: self.rows, cols: self.cols, entries }
    }
    pub fn partition(&self, count: usize) -> Vec<Self> {
        assert!(count > 0);
        let mut parts: Vec<_> = (0..count).map(|p| Self::empty(
            self.rows/count + usize::from(p < self.rows%count), self.cols)).collect();
        for (r,&encoding) in self.row_encoding.iter().enumerate() {
    ...
    pub fn assemble(parts: &[Self]) -> Self {
        ...
        for _ in 0..parts.iter().map(|p| p.row_encoding.len()).sum::<usize>() {
            let p = (0..parts.len()).filter(|&p| cursor[p] < parts[p].row_encoding.len())
                .min_by_key(|&p| (parts[p].row_encoding[cursor[p]],p)).unwrap();
            let r = cursor[p];
            out.append_row(&parts[p],r,(parts[p].row_encoding[r]-1)*parts.len()+p+1);
            cursor[p] += 1;
        }
        out
    }
    ...
    pub fn add<A: Monoid<Element=T>>(&self, b: &Self, algebra: &A) -> Self {
        ...
        while i < self.row_encoding.len() || j < b.row_encoding.len() {
            if j == b.row_encoding.len() || (i < self.row_encoding.len() && self.row_encoding[i] < b.row_encoding[j]) {
                ...
                let row_matrix = |matrix: &Self, row| Csr {
                    rows: 1, cols: matrix.cols, ia: vec![1,matrix.row(row).len()+1],
    ...
    pub fn multiply_dense<A: Semiring<Element=T>>(&self, n: usize, b: &[T],
        alpha: &T, beta: &T, c: Option<&Self>, algebra: &A) -> Self where T: PartialEq {
        assert_eq!(b.len(),self.cols*n);
        if let Some(c) = c { assert_eq!(c.shape(),(self.rows,n)); }
        let columns = if n > 0 && b[(n-1)*self.cols..].iter().all(|v| *v == algebra.zero()) { n-1 } else { n };
        let mut out = Self::empty(self.rows,n);
        if columns > 0 {
            let packed = Csr { rows: self.row_encoding.len(), cols: self.cols,
                ia: self.ia.clone(), ja: self.ja.clone(), values: self.values.clone() };
            let mut dense = vec![algebra.zero(); packed.rows*columns];
            packed.multiply_dense(columns,&b[..self.cols*columns],alpha,&algebra.one(),&mut dense,algebra);
            for (r,&encoding) in self.row_encoding.iter().enumerate() {
