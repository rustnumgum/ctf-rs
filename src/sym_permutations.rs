...
use crate::{
    sym_indices::{align_pair, align_triple},
    symmetry::Symmetry,
};
use Symmetry::{AS, NS};
...
fn normalize<const N: usize>(
        ...
        for operand in 0..N {
            for i in 0..sym[operand].len() {
                ...
                let positions: [Option<usize>; N] = std::array::from_fn(|other| {
                    permutation.indices[other]
                        .iter()
                        .rposition(|&l| l == permutation.indices[operand][i])
                });
                let mut j = i;
                while sym[operand][j] != NS {
                    j += 1;
                    let preserved = (0..N).filter(|&other| other != operand).all(|other| {
                        let right = permutation.indices[other]
                            .iter()
                            .rposition(|&l| l == permutation.indices[operand][j]);
                        match (positions[other], right) {
                            (None, None) => true,
                            (Some(left), Some(right)) => sym[other]
                    ...
                    if preserved
                        && permutation.indices[operand][i] > permutation.indices[operand][j]
                    {
                        permutation.indices[operand].swap(i, j);
                        if sym[operand][i] == AS {
                            permutation.sign = -permutation.sign;
                        }
                        changed = true;
                    }
                }
    ...
    let [a, rest @ ..] = &mut permutation.indices[..] else {
        unreachable!()
    };
    if N == 2 {
...
fn add<const N: usize>(
    permutations: &mut Vec<Permutation<N>>,
    new: Permutation<N>,
    sym: [&[Symmetry]; N],
) {
    let normalized = normalize(new, sym);
    if !permutations.iter().any(|p| p.indices == normalized.indices) {
        permutations.push(normalized);
    }
}
...
pub fn enumerate<const N: usize>(
    ...
    for operand in 0..N {
        ...
        if let Some(last) = sym[operand].last() {
            assert_eq!(*last, NS);
        }
    }
        ...
        for i in 0..indices[operand].len() {
            ...
            while sym[operand][j] != NS {
                ...
                while k < permutations.len() {
                    ...
                    if sym[operand][j - 1] == AS {
                        new.sign = -new.sign;
                    }
                    new.indices[operand].swap(i, j);
                    add(&mut permutations, new, sym);
                    k += 1;
                }
            }
        }
...
pub fn circular_generator(sym: &[Symmetry]) -> (Vec<usize>, usize, i32) {
    ...
    let sign = if order % 2 == 0 && sym[..order].contains(&AS) {
        -1
    } else {
        1
    };
    let permutation = (0..sym.len())
        .map(|i| if i < order { (i + 1) % order } else { i })
        .collect();
