...
#[derive(Debug)]
pub enum BlasContractionError {
    Mapping(crate::map_tensor::Rejected),
    Folding(crate::folding::Rejected),
}
    ...
    A::Element: Wire,
{
        ...
        A: Clone,
    {
        ...
        for (r, &n) in ranges.iter().zip(&self.distribution.shape) {
        ...
        let mut part = source.slice(source_ranges);
        assert_eq!(part.distribution.shape, shape);
        let mut distribution = self.distribution.clone();
        distribution.shape = shape;
        part.redistribute(distribution);
        let send = self
            .distribution
            .shifted_rank(self.context.rank(), &offsets, true);
        let recv = self
            .distribution
            .shifted_rank(self.context.rank(), &offsets, false);
        if send != self.context.rank() && !part.data.is_empty() {
            let mut bytes = Vec::with_capacity(part.data.len() * A::Element::WIDTH);
            ...
            self.context
                .inner
                .send_receive(&bytes, send, recv, &mut received);
            for (value, bytes) in part
                .data
                .iter_mut()
        ...
        for (offset, value) in part.data.iter().enumerate() {
            if let Some(key) = part.distribution.global_key(recv, offset) {
                let coordinates: Vec<_> = part
                    ...
                    .map(|(&c, &o)| c + o)
                    .collect();
                let key = self.distribution.encode_key(&coordinates);
                let index = self.distribution.local_offset(self.context.rank(), key);
                let previous = if *beta == self.algebra.zero() {
                    self.algebra.zero()
                } else {
                    self.algebra.multiply(&self.data[index], beta)
                };
                self.data[index] = self
                    .algebra
                    .add(&self.algebra.multiply(value, alpha), &previous);
            }
        }
    }
        ...
        A: Clone,
    {
        ...
        if !pa.repeated() && !pb.repeated() {
            return self.sum_from_on_grid(indices_b, input, indices_a, topology, alpha, beta);
        }
        ...
        A: Clone,
    {
        ...
        if !pa.repeated() && !pb.repeated() && !pc.repeated() {
            return self.contract_from_on_grid(
                indices_c, a, indices_a, b, indices_b, topology, alpha, beta,
            );
        }
    ...
    pub fn contract_cached(
        ...
        indices_b: &str,
        topology: crate::mapping::Topology,
        cache: &mut crate::planning::PlanCache<'_, '_>,
        alpha: A::Element,
        beta: A::Element,
    ) -> Result<(), crate::map_tensor::Rejected>
    where
    ...
    pub fn contract_from_aligned(
        ...
        for &(indices, d) in &operands {
            assert_eq!(d.topology, self.distribution.topology);
            assert!(indices.is_ascii());
            assert_eq!(indices.len(), d.shape.len());
            for (i, label) in indices.bytes().enumerate() {
                assert!(
                    !indices.as_bytes()[..i].contains(&label),
                    "aligned contraction requires unique labels"
                );
            }
        }
        for i in 0..3 {
        ...
        for i in 0..3 {
            for (map, label) in operands[i].1.mappings.iter().zip(operands[i].0.bytes()) {
                mark(map, label, &mut axes[i]);
            }
        }
        let mut comms: [Vec<Context<'_>>; 3] = std::array::from_fn(|_| Vec::new());
        for axis in 0..topology.dimensions.len() {
            ...
            assert!(
                ...
                "mismatched physical labels require 2D communication"
            );
            for i in 0..3 {
                if axes[i][axis].is_none() {
                    comms[i].push(topology.fiber(self.context, axis));
                }
            }
        }
        let phases: Vec<Vec<_>> = operands
            .iter()
            .map(|(_, d)| {
                d.mappings
                    .iter()
        ...
        let mut bdata = b.data.clone();
        crate::contraction::replicated(
            &self.algebra,
            &comms[0].iter().collect::<Vec<_>>(),
            &comms[1].iter().collect::<Vec<_>>(),
            &comms[2].iter().collect::<Vec<_>>(),
            &shapes[0],
            &phases[0],
            indices_a,
            &mut adata,
        ...
        A: Clone,
    {
        ...
        for (indices, distribution) in [
            ...
            for (i, label) in indices.bytes().enumerate() {
                assert!(
                    !indices.as_bytes()[..i].contains(&label),
                    "unique labels required by this entry point"
                );
                if let Some(j) = labels.iter().position(|&l| l == label) {
                    assert_eq!(shape[j], distribution.shape[i]);
                } else {
                    labels.push(label);
                    shape.push(distribution.shape[i]);
                }
            }
        }
        ...
        let mapped = |indices: &str, shape: &[usize]| {
            Distribution::new(
                ...
                indices
                    .bytes()
                    .map(|label| maps[labels.iter().position(|&l| l == label).unwrap()].clone())
                    .collect(),
            )
        };
        let mut a = Self {
        ...
        b.redistribute(self.distribution.clone());
        self.data = b.data;
        Ok(())
    }
    ...
    pub fn sum_from_aligned(
        ...
        for (indices, distribution) in [
            ...
            for (i, label) in indices.bytes().enumerate() {
                assert!(
                    !indices.as_bytes()[..i].contains(&label),
                    "aligned entry point requires unique labels"
                );
            }
        }
        for (ia, label) in indices_a.bytes().enumerate() {
            if let Some(ib) = indices_b.bytes().position(|x| x == label) {
                ...
                assert_eq!(
                    input.distribution.mappings[ia],
                    self.distribution.mappings[ib]
                );
            }
        }
        ...
        for (map, label) in input.distribution.mappings.iter().zip(indices_a.bytes()) {
            map_labels(map, label, &mut a_axes);
        }
        for (map, label) in self.distribution.mappings.iter().zip(indices_b.bytes()) {
            map_labels(map, label, &mut b_axes);
        }
        ...
        let virtual_a: Vec<_> = input
            .distribution
            .mappings
            .iter()
            .map(|m| m.phase() / m.physical_phase())
            .collect();
        let virtual_b: Vec<_> = self
            .distribution
            .mappings
            .iter()
            .map(|m| m.phase() / m.physical_phase())
        ...
        crate::summation::replicated(
            &self.algebra,
            &input_comms.iter().collect::<Vec<_>>(),
            &output_comms.iter().collect::<Vec<_>>(),
            &input.distribution.block_shape(),
            &virtual_a,
            indices_a,
            &mut a,
            &self.distribution.block_shape(),
            &virtual_b,
            indices_b,
            &mut self.data,
            &alpha,
    ...
    A::Element: Wire,
{
    ...
    pub fn extract_diagonal(&self, labels: &str) -> (Self, String) {
        ...
        let mut result = Self::new(
            self.context,
            Distribution::cyclic(projection.shape.clone(), self.context.size()),
            self.algebra.clone(),
        );
    ...
    pub fn replace_diagonal(&mut self, labels: &str, input: &Self) {
        ...
        for (key, value) in input.local_pairs() {
            if input.distribution.owner(key) != self.context.rank() {
                continue;
            }
        ...
        self.transform_indexed(labels, |value| *value = zero.clone());
    ...
    pub fn slice(&self, ranges: &[std::ops::Range<usize>]) -> Self {
        ...
        Self {
            context: self.context,
            algebra: self.algebra.clone(),
            distribution: plan.output_distribution,
            data,
        }
    }
    ...
    pub fn permute_axes(&self, axes: &[usize]) -> Self {
        ...
        for &axis in axes {
            assert!(axis < order && !seen[axis]);
            seen[axis] = true;
        }
        let distribution = Distribution::new(
            axes.iter().map(|&i| self.distribution.shape[i]).collect(),
            self.distribution.topology.clone(),
            axes.iter()
                .map(|&i| self.distribution.mappings[i].clone())
                .collect(),
        );
        ...
        let storage_order: Vec<_> = axes
            .iter()
            .copied()
            .chain(axes.iter().map(|&axis| axis + order))
            .collect();
    ...
    pub fn write_add(&mut self, pairs: &[(usize, A::Element)]) {
        ...
        for bytes in self.context.inner.exchange(&buckets) {
            for pair in bytes.chunks_exact(8 + A::Element::WIDTH) {
                let key = u64::decode(&pair[..8]) as usize;
                let offset = self.distribution.local_offset(self.context.rank(), key);
                self.data[offset] = self
                    .algebra
                    .add(&self.data[offset], &A::Element::decode(&pair[8..]));
            }
        }
    }
