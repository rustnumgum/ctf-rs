...
impl Topology {
    ...
    pub fn fiber<'a>(&self, context: &'a Context<'_>, axis: usize) -> Context<'a> {
        ...
        let key = coordinates[axis];
        coordinates[axis] = 0;
        context
            .split(
                Some(self.rank(&coordinates).try_into().unwrap()),
                key.try_into().unwrap(),
            )
            .unwrap()
    }
...
impl Mapping {
    ...
    pub fn physical_phase(&self) -> usize {
        match self {
            ...
            Self::Physical {
                processes, child, ..
            } => processes * child.physical_phase(),
    ...
    pub fn physical_rank(&self, coordinates: &[usize]) -> usize {
        match self {
            ...
            Self::Physical {
                ...
                processes,
                child,
            } => coordinates[*axis] + processes * child.physical_rank(coordinates),
...
impl Distribution {
    ...
    pub fn block_shape(&self) -> Vec<usize> {
        self.shape
            .iter()
            .zip(&self.mappings)
            .map(|(&n, m)| n.div_ceil(m.phase()))
            .collect()
    }
    pub fn local_len(&self) -> usize {
        self.block_shape().iter().product::<usize>()
            * self
                .mappings
                .iter()
    ...
    pub fn owner(&self, key: usize) -> usize {
        ...
        for (&c, m) in coordinates.iter().zip(&self.mappings) {
            m.assign_owner(c, &mut owner);
        }
    ...
    pub fn owns(&self, rank: usize, key: usize) -> bool {
        ...
        self.decode_key(key)
            .iter()
            .zip(&self.mappings)
            .all(|(&c, m)| c % m.physical_phase() == m.physical_rank(&rank_coordinates))
    }
    ...
    pub fn global_key(&self, rank: usize, offset: usize) -> Option<usize> {
        ...
        for ((m, &b), &n) in self.mappings.iter().zip(&block).zip(&self.shape) {
            let vp = m.phase() / m.physical_phase();
            let c = (l % b) * m.phase()
                + (v % vp) * m.physical_phase()
                + m.physical_rank(&rank_coordinates);
            if c >= n {
                return None;
            }
            coordinates.push(c);
