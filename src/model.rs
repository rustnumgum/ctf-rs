...
impl LinearModel {
    ...
    pub fn set_coefficients(&mut self, values: &[f64]) {
        assert_eq!(values.len(), self.coefficients.len());
        self.coefficients.copy_from_slice(values);
    }
    pub fn diagnostics(&self) -> &Diagnostics {
        &self.diagnostics
    }
    pub fn retained_observations(&self) -> &[(f64, Vec<f64>)] {
        &self.history
    }
    ...
    pub fn observe(&mut self, seconds: f64, parameters: &[f64]) {
        assert!(seconds >= 0.);
        let predicted = self.estimate(parameters);
        self.totals[0] += seconds;
        if predicted > seconds {
            self.totals[2] += predicted - seconds;
        } else {
            self.totals[1] += seconds - predicted;
        }
    ...
    pub fn update<K: LocalKernels>(&mut self, context: &Context<'_>) -> Result<bool, i32> {
        ...
        let count =
            context.all_reduce(&crate::algebra::Arithmetic::<i64>::new(), &(count as i64)) as usize;
        let tuned = count >= 16 * np * n;
        if tuned {
            let (r, y) = if self.history.len() < n {
                (vec![0.; n * n], vec![0.; n])
            } else {
                let m = self.history.len() + n;
                let mut a = vec![0.; m * n];
                let mut b = vec![0.; m];
                for j in 0..n {
                    a[j + j * m] = if self.coefficients[j] != 0. {
                        1e6f64
                            .min(self.diagnostics.average_total_time / self.coefficients[j] / 1000.)
                    } else {
                        1.
                    };
                }
            ...
            let all_r = comm.inner.all_gather_f64(&r);
            let all_y = comm.inner.all_gather_f64(&y);
            let m = n * np;
            let mut stacked = vec![0.; m * n];
            for rank in 0..np {
                for j in 0..n {
                    for i in 0..n {
...
pub fn cubic_features(parameters: &[f64]) -> Vec<f64> {
    ...
    for i in 0..n {
        for j in 0..=i {
            let square = parameters[i] * parameters[j];
            out.push(square);
            for k in 0..=j {
                cubic.push(square * parameters[k]);
            }
        }
    }
    out.extend(cubic);
...
impl CubicModel {
    pub fn new(
        ...
        assert_eq!(
            ...
            parameters
                ...
                + parameters * (parameters + 1) * (parameters + 2) / 6
        );
        Self {
            parameters,
            linear: LinearModel::new(name, coefficients, history_size),
        }
    }
    ...
    pub fn estimate(&self, parameters: &[f64]) -> f64 {
        assert_eq!(parameters.len(), self.parameters);
        self.linear.estimate(&cubic_features(parameters))
    }
    pub fn observe(&mut self, seconds: f64, parameters: &[f64]) {
        assert_eq!(parameters.len(), self.parameters);
        self.linear.observe(seconds, &cubic_features(parameters));
    }
    pub fn update<K: LocalKernels>(&mut self, context: &Context<'_>) -> Result<bool, i32> {
        self.linear.update::<K>(context)
    }
}
