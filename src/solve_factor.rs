...
macro_rules! solve_factor_body {
    ($self:ident, $output_mode:ident, $factors:ident, $rhs:ident) => {{
        ...
        let mut result = Tensor::new($self.context(), $rhs.distribution().clone(), Arithmetic::new());
...
impl<'c, 'r> Tensor<'c, 'r, Arithmetic<f64>> {
    ...
    pub fn solve_factor(&self, output_mode: usize, factors: &[&Self], rhs: &Self)
        -> Result<Self, i32> {
        solve_factor_body!(self, output_mode, factors, rhs)
    }
}

impl<'c, 'r> SparseTensor<'c, 'r, Arithmetic<f64>> {
    /// Solve sparse weighted normal equations for one dense factor. Stored
    /// entries are weights; absent entries make no contribution to the Gram
    /// systems. Factors and RHS use auxiliary-first `[rank, mode_length]`.
    pub fn solve_factor(
        &self,
        output_mode: usize,
        factors: &[&Tensor<'_, '_, Arithmetic<f64>>],
        rhs: &Tensor<'_, '_, Arithmetic<f64>>,
    ) -> Result<Tensor<'c, 'r, Arithmetic<f64>>, i32> {
        solve_factor_body!(self, output_mode, factors, rhs)
    }
}
