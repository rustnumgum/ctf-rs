...
impl Layout {
    ...
    fn groups(&self) -> impl Iterator<Item = (usize, usize, Symmetry)> + '_ {
        ...
        self.links
            .iter()
            .enumerate()
            .filter_map(move |(end, &link)| {
                if link != Symmetry::NS {
                    return None;
                }
                let group = (start, end + 1, self.links[start]);
                start = end + 1;
                Some(group)
            })
    }
    ...
    pub fn len(&self) -> usize {
        self.groups()
            .map(|(start, end, kind)| {
                group_size(self.shape[start], end - start, kind == Symmetry::SY)
            })
            .product()
    }
    ...
    pub fn symmetric_len(&self) -> usize {
        self.groups()
            .map(|(start, end, _)| group_size(self.shape[start], end - start, true))
            .product()
    }
    ...
    pub fn locate(&self, coordinates: &[usize]) -> Option<(usize, i32)> {
        ...
        for (start, end, kind) in self.groups() {
            ...
            if matches!(kind, Symmetry::AS | Symmetry::SH) && group.windows(2).any(|c| c[0] == c[1])
            {
                return None;
            }
            let mut rank = 0;
            ...
            offset += stride * rank;
            stride *= group_size(self.shape[start], end - start, kind == Symmetry::SY);
        }
        Some((offset, sign))
    }
}
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if self.remaining > 0 {
            ...
            for i in 0..self.current.len() {
                let limit = if self.layout.links[i] == Symmetry::NS {
                    self.layout.shape[i]
                } else {
                    self.current[i + 1] + usize::from(self.layout.links[i] == Symmetry::SY)
                };
                self.current[i] += 1;
                if self.current[i] < limit {
                    break;
                }
                let strict =
                    i > start && matches!(self.layout.links[i - 1], Symmetry::AS | Symmetry::SH);
                self.current[i] = if strict { i - start } else { 0 };
                if self.layout.links[i] == Symmetry::NS {
                    start = i + 1;
                }
            }
        }
...
pub struct Packed<A: Monoid> {
    layout: Layout,
    algebra: A,
    values: Vec<A::Element>,
}
impl<A: Group> Packed<A> {
    ...
    pub fn transform_indexed(&mut self, indices: &str, mut function: impl FnMut(&mut A::Element)) {
        ...
        assert_eq!(indices.len(), self.layout.shape.len());
        for (i, label) in indices.bytes().enumerate() {
            for j in 0..i {
                if indices.as_bytes()[j] == label {
                    assert_eq!(self.layout.shape[i], self.layout.shape[j]);
                }
            }
        }
        for (coordinates, value) in self.layout.coordinates().zip(&mut self.values) {
            let selected = (0..indices.len()).all(|i| {
                (0..i).all(|j| {
                    indices.as_bytes()[i] != indices.as_bytes()[j]
    ...
    pub fn new(layout: Layout, algebra: A) -> Self {
        let values = vec![algebra.zero(); layout.len()];
        Self {
            layout,
            algebra,
            values,
    ...
    pub fn values(&self) -> &[A::Element] {
        &self.values
    }
    pub fn values_mut(&mut self) -> &mut [A::Element] {
        &mut self.values
    }
    pub fn read(&self, coordinates: &[usize]) -> A::Element {
        match self.layout.locate(coordinates) {
            None => self.algebra.zero(),
            Some((offset, 1)) => self.values[offset].clone(),
