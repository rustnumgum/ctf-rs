...
impl Signature {
    pub fn new(distributions: [&Distribution; 3], indices: [&str; 3], topology: Topology) -> Self {
        ...
        Self {
            distributions: distributions.map(Clone::clone),
            indices: normalized,
            topology,
        }
    }
    pub fn distributions(&self) -> &[Distribution; 3] {
        &self.distributions
    }
...
impl GridPlan {
        ...
        fn map(m: &Mapping, out: &mut Vec<u64>) {
            match m {
                ...
                Mapping::Physical {
                    ...
                    out.extend([1, *axis as u64, *processes as u64]);
                    map(child, out);
                }
                Mapping::Virtual { copies, child } => {
                    out.extend([2, *copies as u64]);
                    map(child, out);
                }
            }
        }
        fn vector(v: &[usize], out: &mut Vec<u64>) {
            out.push(v.len() as u64);
            out.extend(v.iter().map(|&x| x as u64));
        }
        fn distribution(d: &Distribution, out: &mut Vec<u64>) {
            vector(&d.shape, out);
            vector(&d.topology.dimensions, out);
            for m in &d.mappings {
                map(m, out);
            }
        }
        let mut out = Vec::new();
        for d in &self.signature.distributions {
            distribution(d, &mut out);
        }
        for indices in &self.signature.indices {
            vector(indices, &mut out);
        }
    ...
    pub(crate) fn unpack(words: &[u64]) -> Self {
        fn value(words: &mut std::slice::Iter<'_, u64>) -> usize {
            (*words.next().unwrap()).try_into().unwrap()
        }
        fn vector(words: &mut std::slice::Iter<'_, u64>) -> Vec<usize> {
            (0..value(words)).map(|_| value(words)).collect()
        }
        fn map(words: &mut std::slice::Iter<'_, u64>) -> Mapping {
            match value(words) {
                ...
                1 => Mapping::Physical {
                    axis: value(words),
                    processes: value(words),
                    child: Box::new(map(words)),
                },
                2 => Mapping::Virtual {
                    copies: value(words),
                    child: Box::new(map(words)),
        ...
        fn distribution(words: &mut std::slice::Iter<'_, u64>) -> Distribution {
            let shape = vector(words);
            let topology = Topology::new(vector(words));
            let maps = (0..shape.len()).map(|_| map(words)).collect();
        ...
        let mapped = std::array::from_fn(|_| distribution(&mut words));
    ...
    fn from_signature(signature: Signature) -> Result<Self, Rejected> {
        ...
        for operand in 0..3 {
            for (axis, &label) in signature.indices[operand].iter().enumerate() {
                assert!(
                    !signature.indices[operand][..axis].contains(&label),
                    "grid plans require unique labels"
                );
                if label == shape.len() {
                    shape.push(signature.distributions[operand].shape[axis]);
                }
            }
        }
        ...
        if !shape.is_empty() {
            crate::map_tensor::assign(
                ...
                &vec![false; shape.len() * shape.len()],
                &mut vec![false; shape.len()],
                &mut maps,
                true,
            )?;
        }
        let mapped = std::array::from_fn(|operand| {
            Distribution::new(
                signature.distributions[operand].shape.clone(),
                signature.topology.clone(),
                signature.indices[operand]
                    .iter()
                    .map(|&label| maps[label].clone())
                    .collect(),
            )
        });
    ...
    pub fn mapped_distributions(&self) -> &[Distribution; 3] {
        &self.mapped
    }
...
impl<'context, 'runtime> PlanCache<'context, 'runtime> {
    ...
    pub fn context(&self) -> &'context Context<'runtime> {
        self.context
    }
