...
impl Candidate {
    fn pack(&self) -> Vec<u64> {
        let mut words = vec![
            self.topology_id as u64,
            u64::from(self.exhaustive),
            self.seconds.to_bits(),
            self.memory_bytes,
        ];
        words.extend(self.plan.pack());
        words
    }
    fn unpack(words: &[u64]) -> Self {
        Self {
            topology_id: words[0] as i64,
            exhaustive: words[1] != 0,
            seconds: f64::from_bits(words[2]),
            memory_bytes: words[3],
            plan: GridPlan::unpack(&words[4..]),
        }
    }
}
...
pub fn replication_factor(distribution: &Distribution) -> usize {
    let physical: usize = distribution
        .mappings
        .iter()
        .map(|m| m.physical_phase())
        .product();
    let virtuals: usize = distribution
        .mappings
        .iter()
...
impl Filter {
    pub fn accepts(self, candidate: &Candidate) -> bool {
        match self {
            ...
            Self::NoReplication(operand) => {
                replication_factor(&candidate.plan.mapped_distributions()[operand]) == 1
            }
        }
    }
    pub fn upstream(name: &str) -> Self {
        match name {
            "max_memory" => Self::MaxMemory(10 * 1024 * 1024),
            "max_time" => Self::MaxTime(0.1),
            "no_replicate_A" => Self::NoReplication(0),
            "no_replicate_B" => Self::NoReplication(1),
            "no_replicate_C" => Self::NoReplication(2),
            _ => panic!("unknown candidate filter"),
        }
    }
}
...
impl<'context, 'runtime> Selector<'context, 'runtime> {
    ...
    pub fn store(&mut self, signature: &Signature, candidate: Candidate) {
        assert_eq!(
            candidate.plan.signature().topology().size(),
            self.context.size()
        );
    ...
    pub fn filter(&mut self, filters: &[Filter]) {
        self.candidates
            .retain(|candidate| filters.iter().all(|&filter| filter.accepts(candidate)));
    }
    ...
    pub fn select_best(&mut self, objective: Objective) -> bool {
        ...
        for candidate in &self.candidates {
            ...
            if candidate.memory_bytes >= objective.memory_limit {
                continue;
            }
            if candidate
                .plan
                ...
                .iter()
                .any(|d| d.local_len() > i32::MAX as usize)
            {
                continue;
            }
        ...
        let mut words = if self.context.rank() == root {
            local.unwrap().pack()
        } else {
            Vec::new()
        };
        let mut size = [words.len() as u64];
        self.context.broadcast(root, &mut size);
        words.resize(size[0].try_into().unwrap(), 0);
        self.context.broadcast(root, &mut words);
        self.selected = Some(Candidate::unpack(&words));
        true
    }
    ...
    pub fn select(&mut self, topology_id: i64, exhaustive: bool) -> bool {
        ...
        let local = self
            .candidates
            .iter()
            .find(|c| c.topology_id == topology_id && c.exhaustive == exhaustive);
        let found = self
            .context
            .inner
            .all_gather_i32(i32::from(local.is_some()));
