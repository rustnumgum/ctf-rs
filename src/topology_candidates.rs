...
pub fn all_shapes(size: usize) -> Vec<Topology> {
    ...
    while remaining > 1 {
        if remaining % divisor == 0 {
            let mut count = 0;
            while remaining % divisor == 0 { remaining /= divisor; count += 1; }
            primes.push(divisor); multiplicities.push(count);
        }
        divisor += 1;
    }
    fn enumerate(primes: &[usize], mults: &[usize], prefix: &mut Vec<usize>, result: &mut Vec<Topology>) {
        let divisors: usize = mults.iter().map(|m|m+1).product();
        assert!(divisors < 1_000_000);
        if divisors == 1 { result.push(Topology::new(prefix.clone())); return; }
...
pub fn peel(topology: &Topology) -> Vec<Topology> {
    ...
    while i < result.len() {
        for candidate in peel(&result[i]) {
            if !result.contains(&candidate) { result.push(candidate); }
        }
        i += 1;
    }
    result
...
pub fn peel_permutations(topology: &Topology) -> Vec<Topology> {
    ...
    while i < permutations.len() {
        ...
        for j in 0..dimensions.len() {
            if dimensions[j] == 2 { continue; }
            for k in 0..dimensions.len() {
                if j == k || dimensions[j] == dimensions[k] { continue; }
                let mut swapped = dimensions.clone(); swapped.swap(j,k);
                let next = Topology::new(swapped);
                if !permutations.contains(&next) { permutations.push(next); }
            }
        }
        i += 1;
    }
