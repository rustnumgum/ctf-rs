...
mpi_tests=(foundation dense_views replicated_sum tensor_sum custom_reduce algebra_sum
  ...
  algebra_contraction tensor_contract contract_remap dense_semantics upstream_dense subcomm_dense plan_cache model_training selector selection_objective tensor_blas_fold upstream_gemm4d distributed_matrix distributed_qr_svd distributed_svd_paths distributed_eigh distributed_spd distributed_tttp distributed_mttkrp distributed_tensor_svd distributed_solve_factor distributed_sparse_io distributed_sparse_sum distributed_sparse_gemm)
args=()
