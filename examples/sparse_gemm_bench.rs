...
use ctf::{algebra::Arithmetic, mapping::Distribution, sparse::SparseTensor};
fn main() {
    ...
    let make = |shape: Vec<usize>, modulus: usize| {
        ...
        let pairs: Vec<_> = (0..shape.iter().product())
            .filter(|&key| key % modulus == 0 && distribution.owner(key) == world.rank())
            .map(|key| (key, ((key % 17) + 1) as f64 / 17.0))
            .collect();
        tensor.write_add(&pairs);
        tensor
    };
    let a = make(vec![m, k], 5);
    let b = make(vec![k, n], 7);
    ...
    if world.rank() == 0 {
        println!(
            "sparse_gemm m={m} k={k} n={n} ranks={np} grid={}x{} elapsed_s={seconds:.6} output_nnz={nnz}",
            grid[0], grid[1]
        );
    }
    drop(c);
