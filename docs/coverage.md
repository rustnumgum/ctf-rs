...
| 4 | sparse_formats/*; contraction/sp*; summation/spr*; tensor sparse paths | sparse; sparse_sum; sparse_gemm; sparse_fold; sparse_diagonal; sparse_formats | speye, sptensor_sum, sparse_mp3, custom sparse endomorphisms | explicit-grid sparse/mixed products with input-only pre-reduction and repeated input/output labels implemented; output-only sparse indices, automatic planning and full upstream sparse suite pending |
