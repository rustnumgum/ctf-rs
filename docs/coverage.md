...
| 4 | interface/{functions,fun_term}; transforms in tensor | algebra; tensor; dense_function; sparse_functions; sparse_function_kernel; sparse_fold_function | univar_function, bivar_function, bivar_transform, endomorphism* | local transforms, typed sparse maps, subset-index accumulators, distributed CSR custom products, fullyfoldable high-order sparse custom contractions and explicitly mapped general dense functions implemented; compressed-symmetry custom contraction dispatch, other kernels and full upstream function tests pending |
