...
| 2 | contraction/{contraction,ctr_tsr,ctr_comm,ctr_2d_general,sym_seq_ctr}; shared/blas_symbs | contraction; sparse_contract_general; ffi/linalg | gemm_4D, weigh_4D, ccsdt_t3_to_t2, sy_times_ns; node-aware execution; S1d Python expressions | dense replication, four-type folded panels, automatic compressed planning and four-type plain-transpose SYR implemented; sparse Hadamard-index elimination precedes planning with source size selection and distributed diagonal remap; S1d WSL/native 1/2/4 PASS; AXPY/SCAL/COPY/DDOT use typed Rust operations |
...
## S1 close target correspondence

| Upstream | Rust driver / implementation | plan.v4 outcome |
|---|---|---|
| contraction/contraction | sparse_contract_general recursive sparse Hadamard-index elimination before raw selection | S1d WSL/native 1/2/4 PASS; planner and kernels unchanged |
| test/python/test_sparse.py | sparse_einsum_hadamard, sparse_scaled_expression, sparse_complex, sparse_sy, sparse_sample | all five WSL 1/2/4 PASS; two changed call sequences native 1/2/4 PASS, other native results retained from S1c |
| examples/checkpoint_sparse.cxx | upstream_checkpoint_sparse, source default n=7 | WSL/native 1/2/4 PASS at unchanged source expression bound 3.43e-6; n=3 history in validation.md |
| examples/algebraic_multigrid.cxx | upstream_algebraic_multigrid, repaired selected layout | WSL 1/2/4 PASS; native S1c passes not repeated |

## Current evidence

C1/S1a/S1b/S1c and CK7/S1d commit series are delivered. Plan.v4's single
thirteen-target WSL set passes at 1/2/4, and all three touched native targets
pass at 1/2/4. The missing sparse ABC entry rewrite was a port gap, now fixed,
not a source restriction. See validation.md "S1d and S1 close" for the complete
fixed-contract acceptance and full-script record. Earlier phase-row HANDOFF
references are superseded for these S1 targets, not for unrelated boundaries.
...
the historical S1a ABC HANDOFF is superseded by the S1d entry rewrite.

S1a implements raw cached folded k1-k5 candidate selection and direct selected
SDD/SSD/SSS execution including nested panels, virtual traversal and output
...
diagnostic; its original distributed gate was HANDOFF at S1a and closed
under the S1d entry rewrite (see "S1d and S1 close"). Two Python ABC
expressions formerly lacked the preplanning rewrite; S1d now accepts them.
