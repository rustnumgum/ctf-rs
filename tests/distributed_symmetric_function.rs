...
fn make<'c, 'r>(context: &'c Context<'r>, shape: Vec<usize>)
...
fn run(context: &Context<'_>) {
    ...
    for physical in ["i", "j", "k"] {
        ...
        output.contract_function_from_on("iji", &a, "ikk", &b, "kj",
            Topology::new(vec![context.size()]), physical, 2, 3, true,
            |a, b| a * a * b * b).unwrap();
        assert_eq!(output.distribution().distribution(), original.distribution());
...
fn main() {
    ...
    child.close();
    if world.rank() == 0 {
        println!("DIGIT / PASS distributed_symmetric_function: exact non-distributive polynomial, repeated labels, physical i/j/k, preserved output layout/off-diagonals; world+parity");
    }
    world.close();
    runtime.finalize();
}
