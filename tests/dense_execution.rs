use ctf::{
    ...
    mapping_variants::VariantSpace,
    normal_mapping::Problem,
    tensor::Tensor,
    topology_candidates,
};

fn a_value(key: usize) -> f64 {
    (key % 7 + 1) as f64
}
fn b_value(key: usize) -> f64 {
    (key % 5) as f64 - 2.
}
...
fn reference(shapes: [&[usize]; 3], indices: [&str; 3], key: usize) -> f64 {
    ...
    let mut coordinates = vec![0; labels.len()];
    let mut remainder = key;
    for (axis, label) in indices[2].bytes().enumerate() {
        coordinates[labels.iter().position(|&l| l == label).unwrap()] = remainder % shapes[2][axis];
        remainder /= shapes[2][axis];
    }
    let reduced: Vec<_> = labels
        .iter()
        .enumerate()
        .filter_map(|(i, &label)| (!indices[2].bytes().any(|l| l == label)).then_some(i))
        .collect();
    let mut result = 0.;
    for flat in 0..reduced.iter().map(|&i| lengths[i]).product() {
        let mut n = flat;
        for &axis in &reduced {
            coordinates[axis] = n % lengths[axis];
            n /= lengths[axis];
        }
        let keys: [usize; 2] = std::array::from_fn(|operand| {
            ...
            let mut key = 0;
            for (axis, label) in indices[operand].bytes().enumerate() {
                key += stride * coordinates[labels.iter().position(|&l| l == label).unwrap()];
                stride *= shapes[operand][axis];
            }
            key
        });
...
fn exercise(c: &Context<'_>, shapes: [&[usize]; 3], indices: [&str; 3], mapped: [Distribution; 3]) {
    let mut a = make(c, shapes[0].to_vec());
    let mut b = make(c, shapes[1].to_vec());
    let mut output = make(c, shapes[2].to_vec());
    a.transform(|key, v| *v = a_value(key));
    b.transform(|key, v| *v = b_value(key));
    output.transform(|_, v| *v = 3.);
    let old = output.distribution().clone();
    output.contract_from_mapped(
        indices[2], &a, indices[0], &b, indices[1], mapped, None, 2., 3.,
    );
    assert_eq!(output.distribution(), &old);
    for (key, value) in output.local_pairs() {
        let expected = 2. * reference(shapes, indices, key) + 9.;
        assert!(
...
fn run(c: &Context<'_>) {
    ...
    for permutation in 0..6 {
        let mapped = problem
            ...
            .unwrap();
        assert!(ctf::mapping_preflight::check(mapped.each_ref(), indices));
        exercise(c, shape, indices, mapped);
    }
    ...
    if c.size() == 4 {
        ...
        for variant in 3..6 {
            exercise(
                ...
                shape,
                indices,
                space.decode(variant).unwrap().distributions,
            );
        }
    }
    ...
    let topology = Topology::new(vec![
        1,
        1,
        if c.size() == 4 { 2 } else { c.size() },
        if c.size() == 4 { 2 } else { 1 },
    ]);
    let p = |axis| Mapping::Physical {
        axis,
        processes: topology.dimensions[axis],
    ...
    if c.size() == 2 {
        maps[1][1].augment_virtual(2);
    }
    let mapped = std::array::from_fn(|o| {
        Distribution::new(shapes[o].to_vec(), topology.clone(), maps[o].clone())
    });
    exercise(c, shapes, ["imkl", "kljn", "imjn"], mapped);

    let topology = Topology::new(vec![c.size()]);
    let k = Mapping::Physical {
        axis: 0,
        processes: c.size(),
        child: Box::new(Mapping::Unmapped),
    };
    let batch = Mapping::Virtual {
        copies: 2,
        child: Box::new(Mapping::Unmapped),
    };
    let shapes: [&[usize]; 3] = [&[3, 3, 3], &[3, 2, 3], &[3, 2, 3]];
    let maps = [
        vec![Mapping::Unmapped, k.clone(), batch.clone()],
        vec![k, Mapping::Unmapped, batch.clone()],
    ...
    let mut a = make(c, vec![3, 3]);
    let mut b = make(c, vec![3, 2]);
    let mut output = make(c, vec![3, 2]);
    a.transform(|key, v| *v = a_value(key));
    b.transform(|key, v| *v = b_value(key));
    output.transform(|_, v| *v = 3.);
    for (iteration, alpha, beta) in [(0, 2., 3.), (1, 3., 0.)] {
        if iteration == 1 {
            a.transform(|key, v| *v = 2. * a_value(key));
        }
        let selected = cache
            .prepare(
        ...
        for (key, value) in output.local_pairs() {
            let expected = if iteration == 0 {
                2. * reference(shape, indices, key) + 9.
            } else {
                6. * reference(shape, indices, key)
            };
    ...
    for (key, value) in output.local_pairs() {
        assert!(value.is_finite() && (value - 6. * reference(shape, indices, key)).abs() < 1e-6);
    }
...
fn main() {
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS dense_execution: raw 2D input/output panels, nested levels, padded fragments, cached search and fresh values; world+parity; abs<1e-6"
        );
    }
    world.close();
    drop(universe);
