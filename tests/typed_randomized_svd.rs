                ...
                for supplied in [false, true] {
                    if !sparse {
                        ...
                        a.transform(|key, x| {
                            let (i, j) = (key % m, key / m);
                            *x = if supplied {
                                value(((key * 17 + 11) % 31) as f64 / 31., (key % 3) as f64 / 7.)
                            } else {
                                value((1 + i + j + 2 * i * j) as f64 / 10., 0.)
                            };
                        });
                        // The automatic case is a rank-two real matrix for all scalar types.
                        let mut guess = make(m, width);
                        guess.transform(|key, x| {
                            *x = value(((key * 7 + 3) % 19) as f64 / 19., (key % 4) as f64 / 11.)
                        });
                        let (u, s, vt) = a
                            .svd_randomized(
                                grid,
                        ...
                        let sparse_source =
                            dense_source.clone().into_sparse(|x| *x != algebra.zero());
                        assert!(sparse_source.local_nnz() < m * n);
                        if context.size() == 4 && context.rank() == 1 {
                            assert!(sparse_source.local_pairs().is_empty());
                        }
                        let mut guess = make(m, width);
                        guess.transform(|key, x| {
                            *x = value(((key * 7 + 3) % 19) as f64 / 19., (key % 4) as f64 / 11.)
                        ...
                        if supplied {
                            ...
                            let (zu, zs, zvt) = sparse_source
                                .svd_randomized(grid, rank, 0, 1, 19, Some(&mut zero_guess))
                                .unwrap();
                            ...
                            check_reconstruction!(
                                ...
                                n,
                                rank,
                                "sparse zero iterations"
                            );
                        }
                        let (u, s, vt) = sparse_source
                            .svd_randomized(
                                grid,
                                rank,
                                1,
...
scalar_case!(real32, f32, |r: f64, _: f64| r as f32, |x: f32| (x as f64)
    .powi(2));
...
fn main() {
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS typed_randomized_svd: four scalar types, dense+sparse randomized SVD, missing-row/empty-shard rank-two fixture, complex plain-transpose projection, oversampled and zero-iteration guess semantics, source Frobenius bound; world+parity"
        );
    }
    world.close();
    drop(universe);
