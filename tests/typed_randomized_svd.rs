    ...
    ($name:ident,$t:ty,$value:expr,$norm2:expr) => {
        fn $name(context:&Context<'_>) {
            ...
            for supplied in [false,true] {
                ...
                let(mut u,s,vt)=a.svd_randomized(grid,rank,1,1,19,if supplied{Some(&mut guess)}else{None}).unwrap();
                ...
                assert!(error[0].sqrt()<=(m*n*n)as f64*1e-6,"randomized residual {} supplied={supplied}",error[0].sqrt());
            }
        }
    }
}
...
fn main(){let runtime=Runtime::initialize();let world=runtime.world();run(&world);
    ...
    if world.rank()==0{println!("DIGIT / PASS typed_randomized_svd: four scalar types, automatic real guesses and complex plain-transpose projection, oversampled guess writeback, source Frobenius bound; world+parity");}
