...
fn run(context: &Context<'_>) {
    let n = 4;
    let make = |order| Tensor::new(context,
        Distribution::cyclic(vec![n; order], context.size()), Arithmetic::<f64>::new());
    let w = make(4);
    let t = make(4);
    let mut z = make(6);
    let topology = Topology::new(if context.size() == 4 {
        vec![2, 2]
    } else { vec![context.size()] });
    z.contract_blas_on_grid::<Native>(
        "hijmno", &w, "hijk", &t, "kmno", topology, 1.0, 1.0,
    ).unwrap();
...
fn main() {
    ...
    child.close();
    if world.rank() == 0 {
        println!("DIGIT / PASS upstream_ccsdt_map: source n=4 zero-input six-order mapping smoke; world+parity");
    }
    world.close();
    runtime.finalize();
}
