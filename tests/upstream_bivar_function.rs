...
fn run(c: &Context<'_>) {
    let mut a = Tensor::new(
        c,
        Distribution::cyclic(vec![6, 5, 7, 8], c.size()),
        Arithmetic::<f64>::new(),
    );
    let mut b = a.clone();
    ...
    for (key, actual) in a.local_pairs() {
        let x = ((key * 17 % 101) as f64 - 50.) / 101.;
        let y = ((key * 29 % 103) as f64 - 51.) / 103.;
...
fn main() {
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS upstream_bivar_function: source four-dimensional identity, deterministic global-key fixture; abs(error)<1e-6; world+parity"
        );
    }
    world.close();
    drop(universe);
