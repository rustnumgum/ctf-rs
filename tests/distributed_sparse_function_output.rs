...
fn main() {
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS distributed_sparse_function_output: source symbolic/numeric CSR custom product, structural zeros, beta clear/merge, world+parity; exact keys and i64"
        );
    }
    world.close();
    drop(universe);
