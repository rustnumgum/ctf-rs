use ctf::{algebra::{Arithmetic,Complex,Monoid,Wire},context::{Context,Runtime},
    mapping::{Distribution,Mapping,Topology},tensor::Tensor};
use std::fmt::Debug;
#[derive(Clone,Debug,PartialEq)]
struct Word([u8;3]);
impl Wire for Word {
    const WIDTH:usize=3;
    fn encode(&self,bytes:&mut Vec<u8>){bytes.extend(self.0);}
    fn decode(bytes:&[u8])->Self{Self(bytes.try_into().unwrap())}
}
#[derive(Clone)]
struct Xor;
impl Monoid for Xor {
    type Element=Word;
    fn zero(&self)->Word{Word([0;3])}
    fn add(&self,a:&Word,b:&Word)->Word{Word(std::array::from_fn(|i|a.0[i]^b.0[i]))}
}
fn layouts(c:&Context<'_>,shape:&[usize])->Vec<Distribution>{
    let top=Topology::new(vec![c.size()]);
    let mut distributions=vec![Distribution::cyclic(shape.to_vec(),c.size()),
        Distribution::new(shape.to_vec(),top.clone(),vec![Mapping::Unmapped;shape.len()])];
    for axis in 0..shape.len(){
        let mut maps=vec![Mapping::Unmapped;shape.len()];
        maps[axis].augment_physical(&top,0);
        maps[axis].augment_virtual(2*c.size());
        for(other,map)in maps.iter_mut().enumerate(){if other!=axis{map.augment_virtual(2);}}
    ...
    if c.size()==4&&!shape.is_empty(){
        ...
        maps[0].augment_physical(&top,1);maps[0].augment_virtual(4);
        ...
        for old in &distributions {
            ...
            for target in &distributions {
                ...
                for(offset,x)in tensor.local_storage().iter().enumerate(){
                    let expected=target.global_key(c.rank(),offset).map_or_else(||algebra.zero(),&value);
...
fn run(c:&Context<'_>){
    ...
    check(c,Arithmetic::<Complex<f64>>::new(),|key|Complex::new(key as f64+0.5,-(key as f64)));
    check(c,Xor,|key|Word([key as u8,key as u8+1,2*key as u8]));
}
fn main(){let runtime=Runtime::initialize();let world=runtime.world();run(&world);
    ...
    if world.rank()==0{println!("DIGIT / PASS cyclic_reshuffle: exact values/padding/replicas, virtual phase changes, empty/scalar, i8/bool/complex/non-Copy Wire; world+parity");}
