...
fn run(c: &Context<'_>) {
    let mut ma = tensor(c, vec![NS, NS]);
    ma.transform(|key, v| *v = (key as f64 - 4.) / 16.);
    let mut mb = tensor(c, vec![NS, NS]);
    mb.transform(|key, v| *v = (2. * key as f64 - 7.) / 32.);
    let mut a = tensor(c, vec![SY, NS, SY, NS]);
    let mut b = tensor(c, vec![SY, NS, SY, NS]);
...
fn main() {
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS upstream_diag_sym: source paired-symmetry diagonal summation identity; norm<1e-10; world+parity"
        );
    }
    world.close();
    drop(universe);
