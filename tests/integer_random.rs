...
use ctf::{
    algebra::Arithmetic, context::{Context, Runtime},
    mapping::{Distribution, Mapping, Topology}, random::Generator,
    symmetric_distribution::SymmetricDistribution, symmetric_tensor::SymmetricTensor,
    symmetry::Symmetry::{SY, AS, SH, NS}, tensor::Tensor,
};
        ...
        fn $name(context: &Context<'_>) {
            for (minimum, maximum) in [(-7 as $scalar, 9 as $scalar), (9, -7), (4, 4)] {
                for shape in [vec![3, 3], vec![1], vec![0], vec![]] {
                    ...
                    let mut tensor = Tensor::new(context, distribution.clone(), Arithmetic::<$scalar>::new());
                    ...
                    for (offset, &actual) in tensor.local_storage().iter().enumerate() {
                        let expected = (reference.unit_interval() * (maximum - minimum) as f64) as $scalar + minimum;
                        assert_eq!(actual, if distribution.global_key(context.rank(), offset).is_some() { expected } else { 0 });
                    }
                ...
                for kind in [SY, AS, SH] {
                    ...
                    let distribution = SymmetricDistribution::new(
                        Distribution::new(vec![3,3], topology, mappings), vec![kind,NS]);
                    let mut valid = vec![false; distribution.local_len()];
                    for (offset, _) in distribution.local_pairs(context.rank()) { valid[offset] = true; }
                    let mut tensor = SymmetricTensor::new(context, distribution, Arithmetic::<$scalar>::new());
                    let mut random = Generator::new(13 * context.rank() as u64);
                    let mut reference = Generator::new(13 * context.rank() as u64);
                    tensor.fill_random(minimum, maximum, &mut random);
                    for (offset, &actual) in tensor.local_storage().iter().enumerate() {
...
cases!(i32_cases, i32);
cases!(i64_cases, i64);
fn run(context: &Context<'_>) { i32_cases(context); i64_cases(context); }
fn main() {
    ...
    if world.rank()==0 { println!("DIGIT / PASS integer_random: exact i32/i64 cast order, draw state, dense/SY/AS/SH, reversed/constant intervals, empty/scalar; world+parity"); }
