    ...
    for kind in [SY, AS, SH] {
        for n in [0, 1, 3] {
            for replicate in [false, true] {
                ...
                let mut parent = SymmetricTensor::new(c, parent_dist, algebra.clone());
                parent.transform(|key, x| *x = value(key as i64 + 1));
                let before = parent.local_storage().to_vec();
                let mut target = child.as_ref().map(|child| {
                    let mut t = SymmetricTensor::new(child, child_dist.clone(), algebra.clone());
                    t.transform(|key, x| *x = value(-(key as i64)));
                    t
                });
                let alpha = value(2);
                let beta = value(3);
                ...
                let down = |key: usize| {
                    algebra.add(
                        &algebra.multiply(&value(key as i64 + 1), &alpha),
                        &algebra.multiply(&value(-(key as i64)), &beta),
                    )
                };
                ...
                if let Some(t) = &target {
                    for (key, x) in t.local_pairs() {
                        assert_eq!(x, down(key));
                    }
                }
                parent.add_from_subworld(
                    target.as_ref(),
                    &child_dist,
                    gamma.clone(),
                    delta.clone(),
                ...
                for (key, x) in parent.local_pairs() {
                    assert_eq!(
                        x,
                        algebra.add(
                            &algebra.multiply(&down(key), &gamma),
                            &algebra.multiply(&value(key as i64 + 1), &delta)
                        )
                    );
                }
                if let Some(t) = &target {
...
fn run(c: &Context<'_>) {
    check(c, Arithmetic::<i64>::new(), |x| x);
    check(c, Arithmetic::<Complex<f64>>::new(), |x| {
        Complex::new(x as f64, (x % 3) as f64)
    });
}
fn main() {
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS symmetric_subworld: exact SY/AS/SH affine accumulation, reversed noncontiguous children, virtual/replicas, empty/scalar-sized domains, integer/complex; world+parity"
        );
    }
    world.close();
    drop(universe);
