...
fn layout(
    ...
    let topology = Topology::new(if replicated {
        vec![context.size()]
...
fn expected(i: usize, j: usize, kind: Symmetry) -> i64 {
    if i == j && kind != Symmetry::SY {
        return 0;
    }
    let value = 1 + i.min(j) as i64 + 10 * i.max(j) as i64;
    if kind == Symmetry::AS && i > j {
        -value
    } else {
        value
    }
...
fn matrix(context: &Context<'_>, n: usize, kind: Symmetry) {
    ...
    let reference: Vec<_> = keys
        .iter()
        .map(|&key| expected(key % n, key / n, kind))
        .collect();
    ...
    for (offset, value) in tensor.local_storage().iter().enumerate() {
        if !valid.iter().any(|&(slot, _)| slot == offset) {
            assert_eq!(*value, 0);
        }
    }
    ...
    assert_eq!(
        restored.read(&keys),
        reference.iter().map(|v| 2 * v).collect::<Vec<_>>()
    );
    ...
    if n > 1 {
        restored.write_add(&[
            (n, 3),
            (n, 4),
            (1, if kind == Symmetry::AS { -5 } else { 5 }),
        ]);
        assert_eq!(
            restored.read(&[n]),
            vec![2 * expected(0, 1, kind) + 12 * context.size() as i64]
        );
    }
}
...
fn triple(context: &Context<'_>) {
    let mut tensor = SymmetricTensor::new(
        context,
        layout(context, 3, 3, Symmetry::AS, false),
        Arithmetic::<i64>::new(),
    );
...
fn main() {
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS distributed_symmetric_io: SY/AS/SH packed holes, parity, replicas, virtual redistribution; exact i64; world+parity"
        );
    }
    world.close();
    drop(universe);
