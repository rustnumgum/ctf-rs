...
use ctf::sparse_formats::{Ccsr, Coo, Csr};
...
fn conversions_and_cyclic_partitions() {
    let coo = Coo::new(
        7,
        5,
        vec![(7, 3, 9i64), (1, 5, 2), (4, 2, 6), (1, 1, 3), (4, 5, 0)],
    );
    let csr = coo.to_csr();
...
fn empty_and_large_logical_rows() {
    for rows in [0, 3] {
        ...
        assert_eq!(
            Ccsr::assemble(&empty.to_ccsr().partition(4)),
            empty.to_ccsr()
        );
...
fn matrices() -> (Csr<i64>, Csr<i64>) {
    ...
    let a = Coo::new(
        4,
        3,
        vec![(1, 1, 1), (1, 3, 2), (3, 2, 3), (4, 1, 4), (4, 3, 5)],
    )
    .to_csr();
...
fn mixed_and_sparse_products_affine() {
    ...
    assert_eq!(
        empty
            .multiply_sparse(&b, &1, &0, None, &algebra)
            .row_offsets(),
        &[1]
    );
}
...
fn sparse_add_and_compressed_product() {
    ...
    assert!(
        a.multiply_dense(1, &[0; 3], &1, &0, None, &algebra)
            .values()
            .is_empty()
    );
    assert!(
        a.multiply_dense(0, &[], &1, &0, None, &algebra)
            .values()
            .is_empty()
    );
}
