...
fn run(c: &Context<'_>) {
    let mut sy = tensor(c, 3, vec![SY, NS], false);
    sy.transform(|key, v| *v = key as f64 + 1.);
    let keys: Vec<_> = (0..9).collect();
    let mut ns = tensor(c, 3, vec![NS, NS], true);
    ns.transform(|_, v| *v = 10.);
    ns.sum_from("ij", &sy, "ij", 2., 3.);
    close(
        ns.read(&keys),
        vec![32., 38., 44., 38., 40., 46., 44., 46., 48.],
    );
    let mut reduced = tensor(c, 0, vec![], true);
    reduced.transform(|_, v| *v = 10.);
    reduced.sum_from("", &sy, "ij", 2., 3.);
    close(reduced.read(&[0]), vec![136.]);
    let mut a = tensor(c, 3, vec![NS, NS], true);
    a.transform(|key, v| *v = key as f64 + 1.);
    let mut projected = tensor(c, 3, vec![SY, NS], false);
    projected.transform(|_, v| *v = 10.);
    projected.sum_from("ij", &a, "ij", 2., 3.);
    // Symmetry-aware sum is an orbit sum, not the canonical-only repack.
    close(
        projected.read(&[0, 3, 4, 6, 7, 8]),
        vec![34., 42., 50., 50., 58., 66.],
    );
    let mut transposed = tensor(c, 3, vec![SY, NS], true);
    transposed.sum_from("ij", &sy, "ji", 1., 0.);
    close(
        transposed.read(&[0, 3, 4, 6, 7, 8]),
        vec![1., 4., 5., 7., 8., 9.],
    );
    let mut anti = tensor(c, 3, vec![AS, NS], true);
    anti.transform(|_, v| *v = 10.);
    anti.sum_from("ij", &sy, "ij", 2., 3.);
    close(anti.read(&[3, 6, 7]), vec![30.; 3]);
    let mut diagonal = tensor(c, 3, vec![NS, NS], false);
    diagonal.transform(|_, v| *v = 10.);
    diagonal.sum_from("ii", &sy, "ii", 2., 3.);
    close(
        diagonal.read(&keys),
        vec![32., 10., 10., 10., 40., 10., 10., 10., 48.],
    );
...
fn main() {
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS distributed_sy_sum: SY unfolding/projection, diagonals, mixed AS, three axes; atol=1e-6; world+parity"
        );
    }
    world.close();
    drop(universe);
