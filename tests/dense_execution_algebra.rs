...
fn exercise<A:Semiring+Clone>(c:&Context<'_>,low_memory:bool,algebra:A,value:impl Fn(usize)->A::Element,
    ...
    for permutation in 0..6{
        ...
        for(key,actual)in b.local_pairs(){assert!(actual==value(key%7+1),"B restored exactly");}
...
fn run(c:&Context<'_>){
    ...
    exercise(c,low_memory,MatrixRing,|n|if n==3{beta.clone()}else{matrix(n)},|a,b|a==b);
    exercise(c,low_memory,Arithmetic::<i64>::new(),|n|n as i64,|a,b|a==b);
    exercise(c,low_memory,Arithmetic::<f32>::new(),|n|n as f32,|a,b|a.is_finite()&&(a-b).abs()<1e-6);
    exercise(c,low_memory,Arithmetic::<Complex<f64>>::new(),|n|Complex::new(n as f64,1.),
