...
fn matrix(context: &Context<'_>, grid: [usize; 2]) {
    ...
    let mut b = Tensor::new(
        context,
        Distribution::cyclic(vec![3, 3], context.size()),
        Arithmetic::<i64>::new(),
    );
    ...
    for beta in [0, 3] {
        ...
        for j in 0..3 {
            for i in 0..5 {
                ...
                if [0, 2, 4].contains(&i) || (beta != 0 && old.iter().any(|(k, _)| *k == key)) {
                    let product: i64 = pairs
                        .iter()
                        .filter(|(key, _)| key % 5 == i)
                        .map(|(key, value)| value * ((key / 5 + 3 * j) as i64 + 1))
                        .sum();
        ...
        let expected: Vec<_> = expected
            .into_iter()
            .filter(|(key, _)| original.owner(*key) == context.rank())
            .collect();
    ...
    let b = Tensor::new(
        context,
        Distribution::cyclic(vec![0, 1], context.size()),
        Arithmetic::<i64>::new(),
    );
...
fn folded(context: &Context<'_>, grid: [usize; 2]) {
    ...
    let mut c = sparse(
        context,
        vec![2, 2, 2],
        &(0..8).map(|key| (key, 7)).collect::<Vec<_>>(),
    );
    let original = c.distribution().clone();
    c.contract_from_sparse_dense("iji", &a, "ikkx", &b, "kjy", grid, 2, 3)
        .unwrap();
    assert_eq!(c.distribution(), &original);
    ...
    for key in 0..8 {
        ...
        if original.owner(key) == context.rank() {
            expected_pairs.push((key, expected));
        }
    }
    let mut actual = c.local_pairs();
    actual.sort_by_key(|pair| pair.0);
...
fn main() {
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS distributed_sparse_dense_output: CCSR represented rows/zeros, beta once, rectangular grids, empty k, repeated labels, input-only sums, restored sparse layout; exact i64; world+parity"
        );
    }
    world.close();
    drop(universe);
