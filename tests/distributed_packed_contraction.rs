use ctf::{
    algebra::Arithmetic,
    context::Context,
    symmetric_contraction::{sequential, sequential_function},
    symmetric_contraction_comm::{replicated, virtualized},
...
fn run(context: &Context<'_>) {
    ...
    let mut c = vec![10; 9];
    sequential(
        &algebra, &sy, "ik", &a, &sy, "kj", &b, &ns, "ij", &mut c, &2, &3,
    );
    ...
    let mut dot = vec![10];
    sequential(
        &algebra, &sy, "ij", &a, &sy, "ij", &b, &scalar, "", &mut dot, &2, &3,
    );
    assert_eq!(dot, vec![254]);
    let mut as_slots = vec![0; 6];
    ...
    let mut rank_a = a
        .iter()
        .map(|v| v + context.rank() as i64)
        .collect::<Vec<_>>();
    let mut rank_b = b.clone();
    let mut total = vec![10];
    replicated(
        &algebra,
        &[],
        &[],
    ...
    if context.rank() == 0 {
        ...
        assert_eq!(total, vec![30 + 224 * np + 27 * np * (np - 1)]);
    }
    let mut broadcast_a = if context.rank() == 0 {
        a.clone()
    } else {
        vec![99; 6]
    };
    let mut broadcast_b = if context.rank() == 0 {
        b.clone()
    } else {
        vec![99; 6]
    };
    let mut product = vec![0; 6];
    replicated(
        &algebra,
        &[context],
        &[context],
    ...
    if context.rank() != 0 {
        assert_eq!(broadcast_a, vec![0; 6]);
        assert_eq!(broadcast_b, vec![0; 6]);
    }
    let va: Vec<_> = (1..=24).collect();
    let vb = vec![2; 24];
    let mut sum = vec![10];
    virtualized(
        &algebra,
    ...
    let empty = Layout::new(vec![0, 0], vec![SY, NS]);
    let mut zero = vec![10];
    sequential(
        &algebra,
        &empty,
...
fn main() {
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS distributed_packed_contraction: canonical kernel, virtual reduction, input broadcasts and root Reduce; exact i64; world+parity"
        );
    }
    world.close();
    drop(universe);
