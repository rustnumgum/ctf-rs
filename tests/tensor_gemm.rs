...
fn main() {
    ...
    let mut a = Tensor::new(
        &world,
        Distribution::cyclic(vec![m, k], np),
        Arithmetic::<f64>::new(),
    );
    let mut b = Tensor::new(
        &world,
        Distribution::cyclic(vec![k, n], np),
        Arithmetic::<f64>::new(),
    );
    ...
    for (key, v) in c.local_pairs() {
        assert_eq!(v, 30. + (2 * k * (key % m + 1) * (key / m + 2)) as f64);
    }
    // Empty local row ownership on multi-rank grids, and a zero reduction length.
    let a0 = Tensor::new(
        &world,
        Distribution::cyclic(vec![1, 0], np),
        Arithmetic::<f64>::new(),
    );
    let b0 = Tensor::new(
        &world,
        Distribution::cyclic(vec![0, 1], np),
        Arithmetic::<f64>::new(),
    );
    let mut c0 = Tensor::new(
        &world,
        Distribution::cyclic(vec![1, 1], np),
        Arithmetic::<f64>::new(),
    );
    c0.transform(|_, v| *v = 5.);
    c0.gemm_2d::<Native>(&a0, &b0, grid, 1., 2.);
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS tensor_gemm: MPI panels + BLAS, uneven dimensions, restored distribution, zero reduction; ranks={np}"
        );
    }
    world.close();
    runtime.finalize();
