...
fn run(context: &Context<'_>) {
    ...
    let mut one = SparseTensor::new(context,Distribution::cyclic(vec![],context.size()),Arithmetic::<f64>::new());
    one.write_add(&if context.rank()==0 {vec![(0,1.0)]}else{vec![]});
