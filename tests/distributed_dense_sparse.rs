...
fn dense<'c, 'r, A: Monoid>(
    c: &'c Context<'r>, d: Distribution, algebra: A, entries: &[(usize, A::Element)],
) -> Tensor<'c, 'r, A>
where A::Element: Wire {
    let mut t = Tensor::new(c, d, algebra);
    t.transform(|key, value| if let Some((_, entry)) = entries.iter().find(|(k, _)| *k == key) {
        *value = entry.clone();
    });
    t
}
...
where A::Element: Wire {
    ...
    let owned: Vec<_> = entries.iter().filter(|(key, _)| d.owner(*key) == c.rank()).cloned().collect();
    t.write_add(&owned);
    t
}
...
fn lookup(values: &[(usize, i64)], key: usize) -> i64 {
    values.iter().find_map(|&(k, v)| (k == key).then_some(v)).unwrap_or(0)
}
...
fn scaled(product: &[i64], old: &[i64]) -> Vec<i64> {
    product.iter().zip(old).map(|(value, old)| 2 * value + 3 * old).collect()
}
...
fn arithmetic(c: &Context<'_>, grid: [usize; 2]) {
    ...
    let b_values = [(0, 2), (1, -3), (3, 5), (6, -11), (7, 13), (8, 17), (10, 19), (11, -23)];
    ...
    let expected_high: Vec<_> = (0..8).map(|key| {
        let j = key % 4;
        let i = key / 4;
    ...
    let mut high = dense(c, high_d.clone(), Arithmetic::<i64>::new(), &entries(&high_old));
    high.contract_from_dense_sparse("ji", &a, "ik", &b, "kj", grid, 2, 3).unwrap();
...
struct Mat([i64; 4]);
impl Mat {
    fn zero() -> Self { Self([0; 4]) }
    fn one() -> Self { Self([1, 0, 0, 1]) }
}
impl Wire for Mat {
    const WIDTH: usize = 32;
    fn encode(&self, output: &mut Vec<u8>) { for value in self.0 { value.encode(output); } }
...
fn mat_mul(a: &Mat, b: &Mat) -> Mat {
    Mat([
        a.0[0] * b.0[0] + a.0[1] * b.0[2], a.0[0] * b.0[1] + a.0[1] * b.0[3],
        a.0[2] * b.0[0] + a.0[3] * b.0[2], a.0[2] * b.0[1] + a.0[3] * b.0[3],
    ])
}
type MatrixAlgebra = CustomSemiring<CustomMonoid<Mat, fn(&Mat, &Mat) -> Mat>, fn(&Mat, &Mat) -> Mat>;
fn matrix_algebra() -> MatrixAlgebra {
    CustomSemiring { monoid: CustomMonoid { identity: Mat::zero(), addition: mat_add }, identity: Mat::one(), multiplication: mat_mul }
}
fn matrix_order(c: &Context<'_>, grid: [usize; 2]) {
    let cd = virtual_first(c, vec![1, 1]);
    let a = dense(c, cyclic(c, vec![1, 1]), matrix_algebra(), &[(0, Mat([0, 1, 0, 0]))]);
    let b = sparse(c, cyclic(c, vec![1, 1]), matrix_algebra(), &[(0, Mat([0, 0, 1, 0]))]);
    ...
    high.contract_from_dense_sparse("ij", &a, "ik", &b, "kj", grid, Mat::one(), Mat::zero()).unwrap();
...
fn main() {
    ...
    run(&parity);
    parity.close();
    if rank == 0 { println!("DIGIT / PASS distributed_dense_sparse: dense*sparse GEMM and contraction, i64 oracle, noncommutative matrix order, world+parity"); }
    world.close();
    runtime.finalize();
}
