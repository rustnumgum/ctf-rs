        ...
        fn $name(context: &Context<'_>) {
            ...
            for (n, degenerate) in [(5, false), (5, true), (1, false)] {
                ...
                let input = |key: usize| {
                    let (i, j) = (key % n, key / n);
                    if degenerate {
                        value(if i == j { (i / 2) as f64 - 1. } else { 0. }, 0.)
                    } else if i == j {
                        value(i as f64 / 4. - 0.5, 0.)
                    } else {
                        value((i + j + 1) as f64 / 20., (i as f64 - j as f64) / 30.)
                    }
                };
                ...
                for (key, x) in a.local_pairs() {
                    assert!(x == input(key), "eigh input unchanged");
                }
                let eigenvalues = values.read(&(0..n).collect::<Vec<_>>());
                assert!(
                    eigenvalues
                        .iter()
                        .all(|&x| norm2(x).is_finite() && x == value(real(x), 0.))
                );
                assert!(
                    eigenvalues
                        .windows(2)
                ...
                for (key, x) in reconstructed.local_pairs() {
                    assert!(norm2(x).is_finite());
                    errors[1] += norm2(algebra.add(&x, &algebra.negate(&input(key))));
                }
                context.sum_f64(&mut errors);
                let bound = (n * n) as f64 * 1e-6;
                assert!(
                    errors.iter().all(|e| e.sqrt() <= bound),
                    "eigh n={n} degenerate={degenerate}: {errors:?} bound={bound}"
                );
            }
        }
    };
...
fn main() {
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS typed_distributed_eigh: four scalar types, Hermitian and degenerate spectra, square subworld and original distribution, source Frobenius bounds; world+parity"
        );
    }
    world.close();
    drop(universe);
