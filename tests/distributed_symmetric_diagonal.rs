...
fn tensor<'c, 'r>(
    ...
    if !maps.is_empty() {
        maps[physical_axis].augment_physical(&topology, 0);
    }
    for map in &mut maps {
...
fn run(c: &Context<'_>) {
    ...
    assert_eq!(
        diagonal.read(&[6, 7, 8, 3, 4, 5]),
        vec![19, 23, 27, -19, -23, -27]
    );
    ...
    assert_eq!(labels, "i");
    assert_eq!(d.read(&[0, 1, 2]), vec![1, 5, 9]);
    d.transform(|_, value| *value *= 2);
    sy.replace_diagonal("ii", &d);
    ...
    assert_eq!(labels, "i");
    assert_eq!(d.read(&[0, 1, 2]), vec![1, 14, 27]);
    for kind in [AS, SH] {
        ...
        assert_eq!(labels, "i");
        assert_eq!(d.read(&[0, 1, 2]), vec![0; 3]);
        d.transform(|_, value| *value = 999);
        hollow.replace_diagonal("ii", &d);
    ...
    let mut b = tensor(c, vec![3, 3], vec![NS, NS], 0);
    b.transform(|_, value| *value = 10);
    b.sum_hollow_from("ii", &a, "iik", 2, 3);
    assert_eq!(
        b.read(&(0..9).collect::<Vec<_>>()),
        vec![52, 10, 10, 10, 68, 10, 10, 10, 84]
    );
}
fn main() {
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS distributed_symmetric_diagonal: extraction/reinsertion, surviving AS links, SY diagonal, virtual mapping; exact i64; world+parity"
        );
    }
    world.close();
    drop(universe);
