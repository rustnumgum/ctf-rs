    ...
    ($name:ident,$t:ty,$value:expr,$conjugate:expr,$norm2:expr,$real:expr) => {
        fn $name(context: &Context<'_>) {
            ...
            for (m, n) in [(13, 7), (5, 8), (1, 1), (1, 3)] {
                ...
                let input = |key: usize| {
                    value(
                        ((key * 17 + 11) % 31) as f64 / 31.
                            + if key % m == key / m { 1. } else { 0. },
                        (key % 3) as f64 / 7.,
                    )
                };
                ...
                let k = m.min(n);
                assert_eq!(u.distribution().shape, vec![m, k]);
                assert_eq!(s.distribution().shape, vec![k]);
                assert_eq!(vt.distribution().shape, vec![k, n]);
                assert_eq!(u.distribution().topology, topology);
                assert_eq!(vt.distribution().topology, topology);
                for (key, x) in a.local_pairs() {
                    assert!(x == input(key), "input preserved");
                }
                ...
                assert!(
                    values
                        .iter()
                        .all(|&x| norm2(x).is_finite() && real(x) >= 0. && x == value(real(x), 0.))
                );
...
fn main() {
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS typed_distributed_svd: four native scalar GESVD families, rectangular/one-row/empty local shards and subcontexts; ordered real singular values, source Frobenius bounds"
        );
    }
    world.close();
    drop(universe);
