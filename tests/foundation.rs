...
fn distributed(world: &Context<'_>) {
    ...
    let mut tensor = Tensor::new(
        world,
        Distribution::cyclic(vec![7, 3], np),
        Arithmetic::<i64>::new(),
    );
    let pairs: Vec<_> = (0..21)
        .filter(|key| key % np == rank)
        .map(|key| (key, key as i64 + 1))
        .collect();
    tensor.write_add(&pairs);
    let requests: Vec<_> = (0..21).rev().filter(|key| (key + 1) % np == rank).collect();
    assert_eq!(
        tensor.read(&requests),
        requests.iter().map(|&k| k as i64 + 1).collect::<Vec<_>>()
    );
    ...
    tensor.redistribute(Distribution::new(
        vec![7, 3],
        topology.clone(),
        vec![Mapping::Unmapped, columns],
    ));
    for (key, value) in tensor.local_pairs() {
        assert_eq!(
            value,
            key as i64 + 1 + if key == 0 { 3 * np as i64 } else { 0 }
        );
    }
    tensor.redistribute(Distribution::new(
        vec![7, 3],
    ...
    assert_eq!(tensor.reduce(), 231 + 3 * np as i64);
    let mut empty = Tensor::new(
        world,
        Distribution::cyclic(vec![7, 0, 7], np),
        Arithmetic::<i64>::new(),
    );
    empty.write_add(&[]);
    assert!(empty.read(&[]).is_empty());
    assert_eq!(empty.reduce(), 0);
    let mut small = Tensor::new(
        world,
    ...
    if rank != 0 {
        assert!(small.local_pairs().is_empty());
    }
    assert_eq!(small.read(&[0]), vec![17]);
    let mut scalar = Tensor::new(
        world,
        Distribution::cyclic(vec![], np),
        Arithmetic::<i64>::new(),
    );
...
fn main() {
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS foundation: exact layouts, algebra, read/write, redistribution, empty shards, subcommunicators; ranks={}",
            world.size()
        );
    }
    world.close();
