...
fn coordinates(key: usize) -> [usize; 3] {
    [key % 3, key / 3 % 3, key / 9]
}
...
fn solution(row: usize, auxiliary: usize) -> f64 {
    0.5 + row as f64 + auxiliary as f64 * 0.25
}
...
fn run(c: &Context<'_>) {
    for mapped_mode in 0..4 {
        ...
        if mapped_mode < 3 {
            mappings[mapped_mode] = mapping;
        }
        let d = Distribution::new(vec![3, 3, 2], topology, mappings);
        let entries: Vec<_> = (0..18)
            .filter(|&key| (weight(key) != 0. || key == 1) && d.owner(key) == c.rank())
            ...
            let factors: Vec<_> = (0..3)
                ...
                .map(|m| {
                    let mut t = Tensor::new(
                        c,
                        Distribution::cyclic(vec![2, d.shape[m]], c.size()),
                        Arithmetic::<f64>::new(),
                    );
                    t.transform(|key, v| {
                        *v = if key % 2 == 0 {
                ...
                .collect();
            let mut rhs = Tensor::new(
                c,
                Distribution::cyclic(vec![2, d.shape[mode]], c.size()),
                Arithmetic::<f64>::new(),
            );
            rhs.transform(|key, v| {
                let row = key / 2;
...
fn main() {
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS distributed_sparse_solve_factor: weighted stored-entry Gram systems, all output and physical modes, virtual blocks, empty shards/systems, world+parity; atol=1e-8 rtol=1e-5"
        );
    }
    world.close();
    drop(universe);
