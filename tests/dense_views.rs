...
fn main() {
    ...
    let mut diagonal = Tensor::new(
        &world,
        Distribution::cyclic(vec![5, 5, 2], world.size()),
        Arithmetic::<i64>::new(),
    );
    ...
    let scalar = Tensor::new(
        &world,
        Distribution::cyclic(vec![], world.size()),
        Arithmetic::<i64>::new(),
    );
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS dense_views: exact slice offsets, virtual blocks, owner shifts, axis transpose, repeated labels, empty slices; ranks={}",
            world.size()
        );
    }
    world.close();
