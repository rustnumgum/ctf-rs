...
fn product<'c, 'r, T: Clone + PartialEq + Wire>(
    a: &Tensor<'c, 'r, Arithmetic<T>>,
    b: &Tensor<'c, 'r, Arithmetic<T>>,
    c: &mut Tensor<'c, 'r, Arithmetic<T>>,
    grid: [usize; 2],
) where
    Arithmetic<T>: Semiring<Element = T> + Clone,
{
    let algebra = Arithmetic::<T>::new();
    c.contract_from_on_grid(
        "ij",
        a,
        "ik",
        b,
...
fn residual<T: Clone + PartialEq + Wire>(
    expected: &Tensor<'_, '_, Arithmetic<T>>,
    actual: &Tensor<'_, '_, Arithmetic<T>>,
    norm: impl Fn(T) -> f64,
    use_actual_norm: bool,
) where
    Arithmetic<T>: ctf::algebra::Group<Element = T>,
{
    let algebra = Arithmetic::<T>::new();
    let mut norms = [0., 0.];
    for ((key, a), (other, b)) in expected.local_pairs().into_iter().zip(actual.local_pairs()) {
        assert_eq!(key, other);
        assert!(norm(b.clone()).is_finite());
        ...
        fn $name(context: &Context<'_>) {
            ...
            for n in [1, 5] {
                ...
                for lower in [true, false] {
                    ...
                    adjoint.transform(|_, x| *x = conjugate(*x));
                    let mut reconstructed = make(n, n);
                    if lower {
                        product(&factor, &adjoint, &mut reconstructed, grid)
                    } else {
                        product(&adjoint, &factor, &mut reconstructed, grid)
                    }
                    residual(&a, &reconstructed, norm, false);
                    let mut forbidden = [0.];
                    for (key, x) in factor.local_pairs() {
                        if (lower && key % n < key / n) || (!lower && key % n > key / n) {
                            forbidden[0] += norm(x).powi(2);
                        }
                    }
                    ...
                    for left in [true, false] {
                        for transpose in [false, true] {
                            let (m, cols) = if left { (n, 3) } else { (3, n) };
                            let mut rhs = make(m, cols);
                            rhs.transform(|key, x| *x = value((key + 1) as f64 / 8., -0.5));
                            let solution = rhs
                                .solve_tri(&factor, grid, lower, left, transpose)
                                .unwrap();
                            ...
                            if left {
                                product(&op, &solution, &mut reconstructed, grid)
                            } else {
                                product(&solution, &op, &mut reconstructed, grid)
                            }
                ...
                for nrhs in [1, 9] {
                    ...
                    rhs.transform(|key, x| *x = value((key % 13 + 1) as f64 / 7., 0.5));
                    let solution = rhs.solve_spd(&a).unwrap();
                    assert_eq!(solution.distribution(), rhs.distribution());
                    let mut reconstructed = make(n, nrhs);
                    product(&a, &solution, &mut reconstructed, grid);
...
scalar_case!(real64, f64, |r: f64, _: f64| r, |x: f64| x, |x: f64| x
    .abs());
scalar_case!(
    complex32,
    Complex<f32>,
    ...
    |x: Complex<f32>| x.norm_squared().sqrt() as f64
);
scalar_case!(
    complex64,
    Complex<f64>,
    ...
    |x: Complex<f64>| x.norm_squared().sqrt()
);
fn run(c: &Context<'_>) {
    real32(c);
    real64(c);
...
fn main() {
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS typed_matrix_factors: native four-type distributed POTRF/POSV/TRSM, Hermitian reconstruction, padding and zero local rows; world+parity; upstream L1<=1e-3 or relative<=1e-3, triangle<=1e-6"
        );
    }
    world.close();
    drop(universe);
