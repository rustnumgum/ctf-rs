    ...
    ($name:ident,$t:ty,$value:expr,$conjugate:expr,$norm2:expr) => {
        fn $name(context: &Context<'_>) {
            ...
            for (randomized, sparse) in [(false, false), (true, false), (false, true), (true, true)]
            {
                ...
                let input = |key: usize| {
                    let (i, j, k) = (key % 3, key / 3 % 2, key / 6);
                    if sparse && key % 3 == 0 {
                        algebra.zero()
                    } else if randomized {
                        value(((i + 1) * (j + 2) * (k + 1)) as f64 / 10., 0.)
                    } else {
                        value(((key * 7 + 3) % 11) as f64 / 10., (key % 4) as f64 / 9.)
                    }
                };
                ...
                let (mut left, s, right) = if sparse {
                    ...
                    let factors = if randomized {
                        sparse_a
                            .tensor_svd_randomized("ijk", "rki", 'r', "jr", grid, 1, 1, 1, 19)
                            .unwrap()
                    } else {
                        sparse_a
                            .tensor_svd_truncated("ijk", "rki", 'r', "jr", grid, None, 0.)
                            .unwrap()
                    };
                    assert_eq!(sparse_a.local_pairs(), original);
                    factors
                } else {
                    a.tensor_svd("ijk", "rki", 'r', "jr", grid, method).unwrap()
                };
                let rank = if randomized { 1 } else { 2 };
                assert_eq!(left.distribution().shape, vec![rank, 2, 3]);
                assert_eq!(s.distribution().shape, vec![rank]);
                assert_eq!(right.distribution().shape, vec![2, rank]);
                for (key, x) in a.local_pairs() {
                    assert!(x == input(key));
                }
                for (factor, labels, renamed) in [(&left, "rki", "ski"), (&right, "jr", "js")] {
                    let mut conjugated = factor.clone();
                    conjugated.transform(|_, x| *x = conjugate(*x));
                    ...
                    for (key, x) in gram.local_pairs() {
                        ...
                        let identity = if key % rank == key / rank {
                            algebra.one()
                        } else {
                            algebra.zero()
                        };
                    ...
                    assert!(
                        error[0] <= 1e-3 || error[0] / rank as f64 <= 1e-3,
                        "tensor SVD orthogonality {}",
                        error[0]
                    );
                }
                ...
                for (key, x) in reconstructed.local_pairs() {
                    assert!(norm2(x).is_finite());
                    error[0] += norm2(algebra.add(&x, &algebra.negate(&input(key))));
                }
                context.sum_f64(&mut error);
                assert!(
                    error[0].sqrt() / 12. < 1e-6,
                    "tensor SVD normalized residual {}",
                    error[0].sqrt() / 12.
                );
            }
        }
...
fn main() {
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS typed_tensor_svd: four scalar types, sparse/dense permuted factor indices, truncated/randomized paths, normalized reconstruction<1e-6 and original orthogonality bounds; world+parity"
        );
    }
    world.close();
    drop(universe);
