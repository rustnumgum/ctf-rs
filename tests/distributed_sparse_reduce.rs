...
use ctf::{
    algebra::{Arithmetic, Monoid, Wire},
    context::Context,
    sparse_formats::Coo,
};
...
fn structural(context: &Context<'_>, root: usize, rows: usize, csr: bool) {
    ...
    let result = if csr {
        matrix
            .to_csr()
            .reduce(context, root, &Arithmetic::<i64>::new())
            .map(|m| m.to_coo())
    } else {
        matrix
            .to_ccsr()
    ...
    if let Some(result) = result {
        ...
        for rank in 0..context.size() {
            for (row, col, value) in entries(rank, rows) {
...
impl Wire for Matrix {
    ...
    fn decode(bytes: &[u8]) -> Self {
        Self(std::array::from_fn(|i| {
            i64::decode(&bytes[i * 8..(i + 1) * 8])
        }))
    }
}
#[derive(Clone)]
struct OrderedProduct;
impl Monoid for OrderedProduct {
    type Element = Matrix;
    fn zero(&self) -> Matrix {
        Matrix([1, 0, 0, 1])
    }
    fn add(&self, a: &Matrix, b: &Matrix) -> Matrix {
        Matrix(std::array::from_fn(|i| {
            let row = i / 2;
            let col = i % 2;
            a.0[2 * row] * b.0[col] + a.0[2 * row + 1] * b.0[2 + col]
        }))
    }
}
fn value(rank: usize) -> Matrix {
    if rank % 2 == 0 {
        Matrix([1, rank as i64 + 1, 0, 1])
    } else {
        Matrix([1, 0, rank as i64 + 1, 1])
    }
}
fn ordered(context: &Context<'_>, root: usize) {
    ...
    let results = [
        matrix
            .to_csr()
            .reduce(context, root, &OrderedProduct)
            .map(|m| m.to_coo()),
        matrix
            .to_ccsr()
            .reduce(context, root, &OrderedProduct)
    ...
    for result in results {
        assert_eq!(result.is_some(), context.rank() == root);
        if let Some(result) = result {
            let expected = (0..context.size()).fold(OrderedProduct.zero(), |old, rank| {
                OrderedProduct.add(&old, &value(rank))
            });
...
fn run(context: &Context<'_>) {
    let mut roots = vec![0, context.size() - 1];
    roots.dedup();
    for root in roots {
        for rows in [0, 5] {
            structural(context, root, rows, true);
            structural(context, root, rows, false);
        }
...
fn main() {
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS distributed_sparse_reduce: CSR/CCSR row partition exchange and recursive reduction, root-only assembly, exact keys/zeros and noncommutative rank order, empty/tall matrices; world+parity"
        );
    }
    world.close();
    drop(universe);
