use ctf::{
    algebra::Arithmetic,
    context::Context,
    symmetric_sum::sequential,
    symmetric_sum_comm::replicated,
...
fn run(context: &Context<'_>) {
    ...
    assert_eq!(
        reduced,
        a.iter()
            .map(|v| 30 + 2 * np * v + np * (np - 1))
            .collect::<Vec<_>>()
    );
    let mut broadcast = if context.rank() == 0 {
        a.clone()
    } else {
        vec![999; 6]
    };
    ...
    assert_eq!(copied, a);
    let mut virtual_input: Vec<_> = (1..=24).collect();
...
fn main() {
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS distributed_packed_sum: packed sequential, virtual, broadcast/reduce; exact i64; world+parity"
        );
    }
    world.close();
    drop(universe);
