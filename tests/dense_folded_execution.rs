...
fn reference(shapes: [&[usize]; 3], indices: [&str; 3], key: usize) -> f64 {
    ...
    let reduced: Vec<_> = labels
        .iter()
        .enumerate()
        .filter_map(|(i, &l)| (!indices[2].bytes().any(|x| x == l)).then_some(i))
        .collect();
    let mut sum = 0.;
...
fn descriptor(mapped: &[Distribution; 3], indices: [&str; 3]) -> Descriptor {
    ...
    let Outcome::Selected(d) = partial_fold::select(
        shapes.each_ref().map(Vec::as_slice),
    ...
    .unwrap() else {
        panic!("expected foldable contraction")
    };
    d
}
fn exercise(c: &Context<'_>, shapes: [&[usize]; 3], indices: [&str; 3], mapped: [Distribution; 3]) {
    let d = descriptor(&mapped, indices);
    let mut a = make(c, shapes[0]);
    let mut b = make(c, shapes[1]);
    let mut output = make(c, shapes[2]);
    a.transform(|key, v| *v = av(key));
    b.transform(|key, v| *v = bv(key));
    output.transform(|_, v| *v = 3.);
    let old = output.distribution().clone();
    output.contract_folded_from_mapped::<Native>(
        indices[2], &a, indices[0], &b, indices[1], mapped, &d, None, 2., 3.,
    );
    ...
    for (key, value) in output.local_pairs() {
        ...
        assert!(
            ...
            "{indices:?}, rank {}, key {key}: {value} != {expected}",
            c.rank()
        );
    }
}
fn run(c: &Context<'_>) {
    let shapes: [&[usize]; 3] = [&[3, 3], &[3, 2], &[3, 2]];
    ...
    exercise(
        ...
        partial,
        ["xik", "kj", "ij"],
        Problem::new(partial, ["xik", "kj", "ij"])
            .unwrap()
            .map_to_topology(&topology, 0, [None; 3])
            .unwrap(),
    );
    if c.size() == 4 {
        let small: [&[usize]; 3] = [&[1, 3], &[3, 1], &[1, 1]];
        ...
        for variant in 3..6 {
            exercise(
                ...
                small,
                indices,
                variants.decode(variant).unwrap().distributions,
            );
        }
    }
    ...
    let l = Mapping::Virtual {
        copies: 2,
        child: Box::new(Mapping::Unmapped),
    };
    let batch: [&[usize]; 3] = [&[3, 3, 3], &[3, 2, 3], &[3, 2, 3]];
    let maps = [
        vec![Mapping::Unmapped, k.clone(), l.clone()],
        vec![k, Mapping::Unmapped, l.clone()],
        vec![Mapping::Unmapped, Mapping::Unmapped, l],
    ];
    exercise(
        c,
        batch,
        ["ikl", "kjl", "ijl"],
        std::array::from_fn(|o| {
            Distribution::new(batch[o].to_vec(), topology.clone(), maps[o].clone())
        }),
    );
    ...
    exercise(
        ...
        ["imkl", "kljn", "imjn"],
        std::array::from_fn(|o| {
            Distribution::new(nested[o].to_vec(), topology.clone(), maps[o].clone())
        }),
    );
    ...
    for factor in [1., 2.] {
        ...
        for (key, value) in output.local_pairs() {
            assert!(
                value.is_finite()
                    && (value - (2. * factor * reference(shapes, indices, key) + 9.)).abs() < 1e-6
            );
        }
    }
    assert_eq!(
...
fn main() {
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS dense_folded_execution: selected/cache fold to BLAS, 2D/nested panels, partial residuals, virtual batches, padded fragments; world+parity; abs<1e-6"
        );
    }
    world.close();
    drop(universe);
