...
fn real<A: Group + Semiring + Clone + CastFromF64>(c: &Context<'_>, algebra: A)
where
    A::Element: Wire + std::fmt::Debug,
{
    ...
    assert_eq!(
        ...
        [2., 8., 14., 8., 10., 16., 14., 16., 18.]
            .into_iter()
            .map(|v| algebra.cast_f64(v))
            .collect::<Vec<_>>()
    );
...
impl Wire for Boxed {
    ...
    fn decode(bytes: &[u8]) -> Self {
        Self(Box::new(f64::decode(bytes)))
    }
}
#[derive(Clone)]
struct BoxedRing;
impl Monoid for BoxedRing {
    type Element = Boxed;
    fn zero(&self) -> Boxed {
        Boxed(Box::new(0.))
    }
    fn add(&self, a: &Boxed, b: &Boxed) -> Boxed {
        Boxed(Box::new(*a.0 + *b.0))
    }
}
impl Group for BoxedRing {
    fn negate(&self, a: &Boxed) -> Boxed {
        Boxed(Box::new(-*a.0))
    }
}
impl Semiring for BoxedRing {
    ...
    fn multiply(&self, a: &Boxed, b: &Boxed) -> Boxed {
        Boxed(Box::new(*a.0 * *b.0))
    }
}
impl CastFromF64 for BoxedRing {
    fn cast_f64(&self, v: f64) -> Boxed {
        Boxed(Box::new(v))
    }
}
...
fn run(c: &Context<'_>) {
    real(c, Arithmetic::<f32>::new());
    real(c, Arithmetic::<i32>::new());
    real(c, Arithmetic::<i64>::new());
    real(c, Arithmetic::<Complex<f32>>::new());
    real(c, BoxedRing);
    complex(c);
}
fn main() {
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS distributed_sy_scalars: f32, integers, complex, non-Copy custom ring; exactly representable fixtures; world+parity"
        );
    }
    world.close();
    drop(universe);
