...
fn sparse<'c, 'r>(dense: &Dense<'c, 'r>) -> Sparse<'c, 'r> {
    ...
    let pairs: Vec<_> = dense
        ...
        .into_iter()
        .filter(|(key, value)| {
            *value != 0 && dense.distribution().owner(*key) == dense.context().rank()
        })
        .collect();
    ...
    for repeated in [false, true] {
        for replicated in [false, true] {
            for x in [0, 2] {
                ...
                let mut a =
                    Dense::new(c, source_distribution(c, sa, replicated), Arithmetic::new());
                let mut b =
                    Dense::new(c, source_distribution(c, sb, replicated), Arithmetic::new());
                a.transform(|key, value| *value = key as i64 % 5 - 2);
                b.transform(|key, value| *value = key as i64 % 7 - 3);
                let aa = sparse(&a);
                let bb = sparse(&b);
                let mut initial = Dense::new(
                    c,
                    Distribution::cyclic(sc.clone(), c.size()),
                    Arithmetic::new(),
                );
                initial.transform(|_, value| *value = 7);
                let mut ss = sparse(&initial);
                let mut sd = initial.clone();
                let mut ds = initial.clone();
                let mut dd = initial.clone();
                let grid = if c.size() == 4 { [2, 2] } else { [c.size(), 1] };
                ss.contract_from(ic, &aa, ia, &bb, ib, grid, 3, 2).unwrap();
                dd.contract_from_sparse(ic, &aa, ia, &bb, ib, grid, 3, 2)
                    .unwrap();
                sd.contract_from_sparse_dense(ic, &aa, ia, &b, ib, grid, 3, 2)
                    .unwrap();
                ds.contract_from_dense_sparse(ic, &a, ia, &bb, ib, grid, 3, 2)
                    .unwrap();
                ...
                for actual in [
                    ...
                    assert_eq!(
                        actual, expected,
                        "repeated={repeated} replicated={replicated} x={x}"
                    );
                }
            }
        }
...
fn main() {
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS distributed_sparse_input_reduction: A/B-only labels, four sparse/mixed paths, repeated indices, empty reduction, replicas, virtual blocks, world+parity; exact i64"
        );
    }
    world.close();
    drop(universe);
