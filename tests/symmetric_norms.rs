...
fn layout(n: usize, kind: Symmetry, np: usize) -> SymmetricDistribution {
    ...
    first.augment_virtual(2 * np);
    let mut second = Mapping::Unmapped;
    second.augment_virtual(2 * np);
    SymmetricDistribution::new(
        Distribution::new(vec![n, n], topology, vec![first, second]),
        vec![kind, NS],
    )
}
            ...
            for kind in [SY, AS, SH, NS] {
                for n in [3, 1] {
                    ...
                    let input = |key| {
                        distribution
                            .canonicalize(key)
                            .map(|(canonical, sign)| ((canonical % 2 + 1) as i32 * sign) as $t)
                            .unwrap_or(0 as $t)
                    };
                    let unpacked = tensor.unpack(Distribution::cyclic(vec![n, n], context.size()));
                    for (key, x) in unpacked.local_pairs() {
                        assert_eq!(x, input(key));
                    }
                    let expected1 = (0..n * n).map(|key| (input(key) as f64).abs()).sum::<f64>();
                    let expected2 = (0..n * n)
                        .map(|key| (input(key) as f64).powi(2))
                        .sum::<f64>()
                        .sqrt();
                    let expected_max = (0..n * n)
                        .map(|key| (input(key) as f64).abs())
                        .fold(0., f64::max);
                    for (actual, expected) in [
                        (tensor.norm1(), expected1),
                        (tensor.norm2(), expected2),
                        (tensor.norm_infty(), expected_max),
                    ] {
                        assert!(
                            actual.is_finite() && (actual - expected).abs() < 1e-6,
                            "symmetric norm {kind:?} n={n} actual={actual} expected={expected}"
                        );
                    }
    ...
    ($name:ident,$t:ty) => {
        fn $name(context: &Context<'_>) {
            for kind in [SY, AS, SH, NS] {
                ...
                let expected = (0..9)
                    .filter(|&key| distribution.canonicalize(key).is_some())
                    .count() as f64
                    * 25.;
                let actual = tensor.norm2();
                assert!(actual.is_finite() && (actual - expected.sqrt()).abs() < 1e-6);
                let unpacked = tensor.unpack(Distribution::cyclic(vec![3, 3], context.size()));
                for (key, x) in unpacked.local_pairs() {
                    let sign = distribution
                        .canonicalize(key)
                        .map(|(_, sign)| sign)
                        .unwrap_or(0) as $t;
...
fn main() {
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS symmetric_norms: SY/AS/SH/NS, source typed-square vs manual norm2, exact distributed unpack, virtual/padded/empty shards; finite abs<1e-6, world+parity"
        );
    }
    world.close();
    drop(universe);
