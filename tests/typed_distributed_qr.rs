    ...
    ($name:ident,$t:ty,$value:expr,$conjugate:expr,$norm2:expr) => {
        fn $name(context: &Context<'_>) {
            ...
            for (m, n) in [(13, 7), (5, 8), (1, 1)] {
                ...
                let k = m.min(n);
                assert_eq!(q.distribution().shape, vec![m, k]);
                assert_eq!(r.distribution().shape, vec![k, n]);
                ...
                let mut adjoint = q.permute_axes(&[1, 0]);
...
fn main() {
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS typed_distributed_qr: four native scalar GEQRF/ORGQR/UNGQR families, rectangular/empty local shards and subcontexts; source Frobenius residual/orthogonality bounds"
        );
    }
    world.close();
    drop(universe);
