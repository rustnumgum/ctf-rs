...
macro_rules! scalar_case {
    ($name:ident,$t:ty,$convert:expr) => {
        fn $name(context: &Context<'_>, token: u64, scope: usize) {
            let value = $convert;
            let entries = [(0usize, 2), (5, 3), (0, -1), (11, 4), (7, -2)];
            ...
            for with_values in [false, true] {
                for reverse in [false, true] {
                    let base = std::env::temp_dir().join(format!(
                        "ctf-rs-io-{token}-{scope}-{}-{with_values}-{reverse}",
                        stringify!($name)
                    ));
                    let input = base.with_extension("input.txt");
                    ...
                    if context.rank() == 0 {
                        ...
                        for &(key, x) in &entries {
                            ...
                            write!(&mut text, "{a} {b}").unwrap();
                            if with_values {
                                write!(&mut text, " {}", value(x)).unwrap();
                            }
                            text.push('\n');
                        }
                    ...
                    let expected = |key: usize| {
                        let mut x = if key == 2 { 1 as $t } else { 0 as $t };
                        for &(k, n) in &entries {
                            if k == key {
                                x += if with_values { value(n) } else { 1 as $t };
                            }
                        }
                        x
                    };
                    for sparse_case in [false, true] {
                        ...
                        if sparse_case {
                            ...
                            tensor
                                .transform(|key, x| *x = if key == 2 { 1 as $t } else { 0 as $t });
                            tensor.read_sparse_from_file(&input, with_values, reverse);
                            for (key, x) in tensor.local_pairs() {
                                assert_eq!(x, expected(key));
                            }
                        ...
                        for (key, x) in restored.local_pairs() {
                            assert_eq!(
                                x,
                                if with_values {
                                    expected(key)
                                } else {
                                    (expected(key) != 0 as $t) as u8 as $t
                                }
                            );
                        }
                        context.barrier();
...
fn empty(context: &Context<'_>, token: u64, scope: usize) {
    ...
    let mut read = SparseTensor::new(context, distribution, Arithmetic::<i64>::new());
    read.read_sparse_from_file(&path, true, false);
    assert_eq!(read.local_nnz(), 0);
    context.barrier();
    if context.rank() == 0 {
...
fn main() {
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS sparse_text_io: four scalar families, distributed dense/sparse additive reads and writes, duplicate keys, reverse/no-value records, tiny/empty files; exact fixtures, world+parity"
        );
    }
    world.close();
    drop(universe);
