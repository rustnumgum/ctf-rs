...
use ctf::{algebra::Arithmetic, context::{Context,Runtime}, mapping::{Distribution,Mapping,Topology},
    symmetry::Symmetry::{self,*}, symmetric_distribution::SymmetricDistribution,
    symmetric_tensor::SymmetricTensor};
fn distribution(context:&Context<'_>, links:Vec<Symmetry>, replicas:bool)->SymmetricDistribution {
    let topology=Topology::new(vec![context.size()]);
    let mut mappings=vec![Mapping::Unmapped;links.len()];
    if !replicas {mappings[0].augment_physical(&topology,0);}
    for mapping in &mut mappings {mapping.augment_virtual(context.size()*2);}
    SymmetricDistribution::new(Distribution::new(vec![3;links.len()],topology,mappings),links)
}
fn run(context:&Context<'_>) {
    let mut ns=SymmetricTensor::new(context,distribution(context,vec![NS,NS],false),Arithmetic::<i64>::new());
    ...
    for kind in [SY,AS,SH] {
        ...
        let expected:Vec<_>=keys.iter().map(|&key| {
            ...
            if i==j && kind!=SY {0} else if i>j && kind==AS {-value} else {value}
        }).collect();
        assert_eq!(packed.read(&keys),expected);
        let unpacked=packed.repack_to(distribution(context,vec![NS,NS],false));
        let chamber:Vec<_>=keys.iter().map(|&key| {
            let i=key%3;let j=key/3;
            if i<j || (i==j && kind==SY) {key as i64+1} else {0}
        }).collect();
        assert_eq!(unpacked.read(&keys),chamber);
    }
    let mut triple=SymmetricTensor::new(context,distribution(context,vec![NS,NS,NS],false),Arithmetic::<i64>::new());
    ...
    for kind in [SY,AS] {
        ...
        let expected:Vec<_>=keys.iter().map(|&key| {
            let i=key%3;let j=key/3%3;let k=key/9;
            if (kind==SY && i<=j && j<=k) || (kind==AS && i<j && j<k) {key as i64+1} else {0}
        }).collect();
...
fn main() {
    ...
    if world.rank()==0 {println!("DIGIT / PASS distributed_symmetric_repack: canonical intersection, SY/AS/SH, partial groups, replicas; exact i64; world+parity");}
