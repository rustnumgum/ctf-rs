...
fn value(key: usize, seed: usize) -> f64 {
    ((key * seed % 101) as f64 - 50.) / 101.
}
fn run(c: &Context<'_>) {
    let d = Distribution::cyclic(vec![6, 5, 7, 8], c.size());
    let mut a = Tensor::new(c, d.clone(), Arithmetic::<f64>::new());
    let mut b = a.clone();
    let mut output = a.clone();
    a.transform(|key, v| *v = value(key, 17));
    b.transform(|key, v| *v = value(key, 29));
    output.transform(|key, v| *v = value(key, 37));
    ...
    let mut integers = Tensor::new(
        c,
        Distribution::cyclic(vec![2], c.size()),
        Arithmetic::<i64>::new(),
    );
    integers.transform(|key, v| *v = key as i64 + 1);
    let mut scalar = Tensor::new(
        c,
        Distribution::cyclic(vec![], c.size()),
        Arithmetic::<f64>::new(),
    );
    scalar.transform(|_, v| *v = 0.5);
    let mut diagonal = Tensor::new(
        c,
        Distribution::cyclic(vec![2, 3, 3], c.size()),
        Arithmetic::<f64>::new(),
    );
    diagonal.transform(|_, v| *v = 5.);
    diagonal.transform_from("ijj", &integers, "i", &scalar, "", |a, b, c| {
        *c += *a as f64 * b
    });
    for (key, actual) in diagonal.local_pairs() {
        let expected = if key / 2 % 3 == key / 6 {
            5. + (key % 2 + 1) as f64 * 0.5
        } else {
            5.
        };
...
fn main() {
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS upstream_bivar_transform: original mutable-output identity, deterministic global-key values, abs(error)<1e-6; world+parity"
        );
    }
    world.close();
    drop(universe);
