...
fn run(context: &Context<'_>) {
    ...
    assert_eq!(
        hollow.read(&keys),
        anti_expected.iter().map(|v| v.abs()).collect::<Vec<_>>()
    );
...
fn main() {
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS distributed_symmetric_operations: indexed scaling, group repack, scaled writes; exact i64; world+parity"
        );
    }
    world.close();
    drop(universe);
