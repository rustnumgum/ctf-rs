...
fn matrices(c:&Context<'_>) {
    ...
    for kind in [AS,SH] {
        let mut a=tensor(c,3,vec![kind,NS],false);a.transform(|key,value|*value=key as i64+1);
        let mut ns=tensor(c,3,vec![NS,NS],true);ns.transform(|_,value|*value=10);
        ns.sum_hollow_from("ij",&a,"ij",2,3);
        let expected:Vec<_>=keys.iter().map(|&key| {
            let i=key%3;let j=key/3;
            if i==j {30} else {let sign=if i>j && kind==AS {-1}else{1};30+2*sign*(i.min(j)+3*i.max(j)+1) as i64}
        }).collect();
        ...
        assert_eq!(transposed.read(&[3,6,7]),vec![4*sign,7*sign,8*sign]);
        let mut reduced=tensor(c,0,vec![],true);reduced.transform(|_,value|*value=10);
        reduced.sum_hollow_from("",&a,"ij",2,3);
        assert_eq!(reduced.read(&[0]),vec![if kind==AS{30}else{106}]);
        let other=if kind==AS{SH}else{AS};
        let mut mismatch=tensor(c,3,vec![other,NS],true);mismatch.transform(|_,value|*value=10);
        mismatch.sum_hollow_from("ij",&a,"ij",2,3);
        assert_eq!(mismatch.read(&[3,6,7]),vec![30;3]);
    }
    let mut a=tensor(c,3,vec![NS,NS],true);a.transform(|key,value|*value=key as i64+1);
    for kind in [AS,SH] {
        let mut out=tensor(c,3,vec![kind,NS],false);out.transform(|_,value|*value=10);
        out.sum_hollow_from("ij",&a,"ij",2,3);
        assert_eq!(out.read(&[3,6,7]),if kind==AS{vec![34,38,34]}else{vec![42,50,58]});
    }
}
fn triples(c:&Context<'_>) {
    ...
    for kind in [AS,SH] {
        ...
        assert_eq!(expanded.read(&[21,15,19,7,11,5,0]),
            if kind==AS {vec![7,-7,-7,7,7,-7,0]}else{vec![7,7,7,7,7,7,0]});
    }
}
fn run(c:&Context<'_>){matrices(c);triples(c);}
fn main(){let runtime=Runtime::initialize();let world=runtime.world();run(&world);
    ...
    if world.rank()==0{println!("DIGIT / PASS distributed_hollow_sum: unfolding, projection, AS cancellation, SH factors; exact i64; world+parity");}
