//! Port of pinned CTF `studies/fast_sy_as_as_tensor_ctr.cxx` (SY <- AS * AS).
...
fn run(context: &Context<'_>) -> (f64, f64, f64) {
    ...
    fill_antisymmetric(&mut b, 21);
    let (b_is_antisymmetric, b_antisymmetry_norm) = check_symmetry(&b, true);
    assert!(
        b_is_antisymmetric,
        "source B antisymmetry norm={b_antisymmetry_norm}"
    );
    if T + V > 1 {
        assert!(
            !check_symmetry(&b, false).0,
            "source B unexpectedly symmetric"
        );
    }
    ...
    let (c_is_symmetric, c_symmetry_norm) = check_symmetry(&c_answer, false);
    assert!(
        c_is_symmetric,
        "source C_ans symmetry norm={c_symmetry_norm}"
    );
    ...
    let (zb_is_antisymmetric, zb_symmetry_norm) = check_symmetry(&z_b_operands, true);
    assert!(
        zb_is_antisymmetric,
        "source Z_B_ops antisymmetry norm={zb_symmetry_norm}"
    );
    contract(
        &mut z_products,
        &idx_z,
    ...
    let (z_is_symmetric, z_symmetry_norm) = check_symmetry(&z_products, false);
    assert!(
        z_is_symmetric,
        "source Z_mults symmetry norm={z_symmetry_norm}"
    );
    c.sum_from(
        &text(&idx_c),
        &z_products,
        ...
        for p in V.saturating_sub(T + r)..=V - r {
            for q in V.saturating_sub(S + r)..=V - p - r {
                ...
                for selected in subsets(&idx_c, T + V - q - r) {
                    let coefficient = sign(subset_parity(&selected, &idx_c));
                    let mut idx_vbb = idx_kr.clone();
                    idx_vbb.extend(&idx_kq);
                    idx_vbb.extend(selected);
                    vb_operands.sum_from(&text(&idx_vb), &b, &text(&idx_vbb), coefficient, 1.0);
                }
                contract(
                    &mut correction_v,
                    &idx_c,
    ...
    let (c_is_symmetric, c_algorithm_symmetry_norm) = check_symmetry(&c, false);
    assert!(
        c_is_symmetric,
        "source C symmetry norm={c_algorithm_symmetry_norm}"
    );
    let (v_is_symmetric, v_symmetry_norm) = check_symmetry(&correction_v, false);
    assert!(v_is_symmetric, "source V symmetry norm={v_symmetry_norm}");
    let (w_is_symmetric, w_symmetry_norm) = check_symmetry(&correction_w, false);
    assert!(w_is_symmetric, "source W symmetry norm={w_symmetry_norm}");
...
fn main() {
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS upstream_fast_sy_as_as_tensor_ctr: n=4,s=t=v=1; internal-AS/SY world={:e} parity={:e} <=1e-6; final world={:e} parity={:e} <=1e-3",
