...
// Adapted from pinned CTF examples/sparse_mp3.cxx, sparse-T path.
//
...
impl Wire for DPair {
    ...
    fn decode(input: &[u8]) -> Self {
        Self {
            a: f64::decode(&input[..8]),
            b: f64::decode(&input[8..16]),
        }
    }
}

fn add_pair(a: &DPair, b: &DPair) -> DPair {
    DPair {
        a: a.a + b.a,
        b: a.b + b.b,
    }
...
fn pair_algebra() -> CustomMonoid<DPair, fn(&DPair, &DPair) -> DPair> {
    CustomMonoid {
        identity: DPair { a: 0.0, b: 0.0 },
        addition: add_pair,
    }
}
...
fn mp3_sparse_t<'c, 'r>(
    ...
    t_pair.accumulate_from_dense("abij", ei, "j", |input, output| {
        output.b += *input;
    });
    t_pair.accumulate_from_dense("abij", ea, "a", |input, output| {
        output.b -= *input;
    });
    t_pair.accumulate_from_dense("abij", ea, "b", |input, output| {
        output.b -= *input;
    });
    let t = t_pair.map_stored(F64::new(), |pair| pair.a / pair.b);

    let mut z = Tensor::new(
        ea.context(),
        Distribution::cyclic(vec![NV, NV, NO, NO], ea.context().size()),
        F64::new(),
    );
    z.sum_from_sparse("abij", vijab, "ijab", 1.0, 0.0);
    z.contract_from_dense_sparse("abij", fab, "af", &t, "fbij", grid, 1.0, 1.0)
        .unwrap();
    z.contract_from_dense_sparse("abij", fij, "ni", &t, "abnj", grid, -1.0, 1.0)
        .unwrap();
    z.contract_from_sparse("abij", vabcd, "abef", &t, "efij", grid, 0.5, 1.0)
        .unwrap();
    z.contract_from_sparse("abij", vijkl, "mnij", &t, "abmn", grid, 0.5, 1.0)
        .unwrap();
    z.contract_from_sparse("abij", vaibj, "amei", &t, "ebmj", grid, 1.0, 1.0)
        .unwrap();
    divide_ea_ei(ea, ei, &mut z, topology);
...
fn run(context: &Context<'_>) -> (f64, f64, f64) {
    ...
    let sparse_energy = mp3_sparse_t(
        &ea,
        &ei,
        &fab,
        &fij,
    ...
    assert!(
        relative < 1.0e-6,
        "dense/sparse-T MP3 mismatch: dense={dense_energy:e} sparse={sparse_energy:e} relative={relative:e}"
    );
