...
fn run(c: &Context<'_>) {
    let mut a = tensor(c, vec![3, 3], vec![SY, NS], false);
    a.transform(|key, v| *v = key as i64 + 1);
    let mut b = tensor(c, vec![3, 3], vec![NS, NS], true);
    b.transform(|_, v| *v = 10);
    b.sum_canonical_from("ij", &a, "ij", 2, 3);
    assert_eq!(
        b.read(&(0..9).collect::<Vec<_>>()),
        vec![32, 30, 30, 38, 40, 30, 44, 46, 48]
    );
    let mut rows = tensor(c, vec![3], vec![NS], false);
    rows.transform(|_, v| *v = 10);
    rows.sum_canonical_from("i", &a, "ij", 2, 3);
    assert_eq!(rows.read(&[0, 1, 2]), vec![54, 56, 48]);
    let mut scalar = tensor(c, vec![], vec![], true);
    scalar.transform(|_, v| *v = 10);
    scalar.sum_canonical_from("", &a, "ii", 2, 3);
    assert_eq!(scalar.read(&[0]), vec![60]);
    let mut diagonal = tensor(c, vec![3, 3], vec![SY, NS], false);
    diagonal.transform(|_, v| *v = 10);
    diagonal.sum_canonical_from("ii", &rows, "i", 2, 3);
    assert_eq!(
        diagonal.read(&[0, 3, 4, 6, 7, 8]),
        vec![138, 10, 142, 10, 10, 126]
    );
    let mut vector = tensor(c, vec![3], vec![NS], true);
    vector.transform(|key, v| *v = key as i64 + 1);
    let mut broadcast = tensor(c, vec![3, 3], vec![AS, NS], false);
    broadcast.sum_canonical_from("ij", &vector, "i", 1, 0);
    assert_eq!(
        broadcast.read(&(0..9).collect::<Vec<_>>()),
        vec![0, -1, -1, 1, 0, -2, 1, 2, 0]
    );
    ...
    scalar.sum_canonical_from("", &empty, "ij", 2, 3);
    assert_eq!(scalar.read(&[0]), vec![180]);
}
fn main() {
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS distributed_canonical_sum: packed tensor routing, reductions, broadcast, diagonals, replicas; exact i64; world+parity"
        );
    }
    world.close();
    drop(universe);
