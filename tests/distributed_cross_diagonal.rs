...
fn tensor<'c, 'r>(
    ...
    maps[2].augment_physical(&topology, 0);
    for map in &mut maps {
...
fn run(c: &Context<'_>) {
    let mut a = tensor(c, vec![NS, AS, NS]);
    a.transform(|key, v| *v = key as f64 + 1.);
    let before = a.read(&(0..27).collect::<Vec<_>>());
    let (mut d, labels) = a.extract_diagonal("iji");
    assert_eq!(labels, "ij");
    assert_eq!(d.distribution().links(), &[NS, NS]);
    close(
        d.read(&(0..9).collect::<Vec<_>>()),
        vec![0., 11., 21., -10., 0., 24., -19., -23., 0.],
    );
    d.transform(|_, v| *v *= 2.);
    a.replace_diagonal("iji", &d);
...
fn main() {
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS distributed_cross_diagonal: cross-AS source run_diag extraction/reinsertion; atol=1e-6; world+parity"
        );
    }
    world.close();
    drop(universe);
