use ctf::{algebra::Arithmetic,context::{Context,Runtime},mapping::{Distribution,Topology},
    planning::PlanCache,sparse::SparseTensor,tensor::Tensor};
fn run(context:&Context<'_>){
    ...
    let topologies=[Topology::new(vec![context.size()]),
        Topology::new(if context.size()==4{vec![2,2]}else{vec![1,context.size()]})];
    for topology in topologies{
        let mut a=SparseTensor::new(context,da.clone(),Arithmetic::<i64>::new());
        let entries:Vec<_>=(0..15).filter(|&key|key%3!=0&&da.owner(key)==context.rank())
        ...
        for empty in [false,true]{
            ...
            let plan=cache.prepare([a.distribution(),b.distribution(),c.distribution()],
    ...
    assert_eq!(cache.len(),2);assert_eq!(cache.stats().misses,2);assert_eq!(cache.stats().hits,2);
}
fn main(){let runtime=Runtime::initialize();let world=runtime.world();run(&world);
    ...
    if world.rank()==0{println!("DIGIT / PASS distributed_sparse_plan: existing GridPlan/PlanCache reuse, changing nnz/values, topology switch, restored distribution; exact i64; world+parity");}
