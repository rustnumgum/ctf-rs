...
fn run(context: &Context<'_>) {
    if context.size() == 4 {
        ...
        let p = Mapping::Physical {
            axis: 0,
            processes: 2,
            child: Box::new(Mapping::Unmapped),
        };
        let q = Mapping::Physical {
            axis: 1,
            processes: 2,
            child: Box::new(Mapping::Unmapped),
        };
        let mapped = [
            Distribution::new(vec![8], topo.clone(), vec![p.clone()]),
            Distribution::new(vec![8, 2], topo.clone(), vec![p, q.clone()]),
            Distribution::new(vec![2], topo, vec![q]),
        ];
        let models = Models::upstream(1);
        let choice = node_reordering::select_dense(
            mapped.each_ref(),
            ["i", "ij", "j"],
            &[0., 1.],
            2,
        ...
        let mut a = Tensor::new(
            context,
            Distribution::cyclic(vec![8], 4),
            Arithmetic::<f64>::new(),
        );
        let mut b = Tensor::new(
            context,
            Distribution::cyclic(vec![8, 2], 4),
            Arithmetic::<f64>::new(),
        );
        let mut c = Tensor::new(
            context,
            Distribution::cyclic(vec![2], 4),
            Arithmetic::<f64>::new(),
        );
        ...
        c.contract_folded_from_mapped::<Native>(
            ...
            mapped,
            &fold,
            Some(&choice.intra_node_lens),
            2.,
            3.,
        );
        for (_, value) in c.local_pairs() {
            assert!(value.is_finite() && (value - 153.).abs() < 1e-6);
        }
    }
    ...
    for permutation in 0..6 {
        ...
        let mut a = Tensor::new(
            context,
            Distribution::cyclic(shapes[0].to_vec(), context.size()),
            Arithmetic::<f64>::new(),
        );
        let mut b = Tensor::new(
            context,
            Distribution::cyclic(shapes[1].to_vec(), context.size()),
            Arithmetic::<f64>::new(),
        );
        let mut c = Tensor::new(
            context,
            Distribution::cyclic(shapes[2].to_vec(), context.size()),
            Arithmetic::<f64>::new(),
        );
        a.transform(|key, v| *v = (key + 1) as f64);
        ...
        for (key, value) in c.local_pairs() {
            ...
            let expected = 2.
                * (0..5)
                    .map(|k| ((i + 3 * k + 1) * ((k + 5 * j) % 7 + 1)) as f64)
                    .sum::<f64>()
                + 3. * (key + 2) as f64;
            assert!(
    ...
    let forward: Vec<_> = (0..6)
        .map(|r| node_reordering::reorder_rank(&[2, 3], &[1, 3], r))
        .collect();
    let backward: Vec<_> = (0..6)
        .map(|r| node_reordering::inverse_rank(&[2, 3], &[1, 3], r))
        .collect();
...
fn main() {
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS distributed_node_fold: rank permutation exact; six raw folded maps and backmapping abs<1e-6; world+parity"
        );
    }
    world.close();
    drop(universe);
