...
fn compressed_group_transpose_does_not_expand_symmetry(){
    ...
    assert_eq!(layout.group_lengths,[3,6,2]);
    layout.permute_folded(&[1,0]);assert_eq!(layout.inner_ordering,[2,1,0]);
    let input:Vec<_>=(0..72).map(|i|format!("v{i}")).collect();
    let packed=layout.transpose(&input,2,Direction::Forward);assert_eq!(packed.len(),72);
    for virtual_block in 0..2{for a in 0..3{for b in 0..6{for c in 0..2{
        assert_eq!(packed[virtual_block*36+c+2*(b+6*a)],input[virtual_block*36+a+3*(b+6*c)]);
    }}}}
    assert_eq!(layout.transpose(&packed,2,Direction::Backward),input);
}
#[test]
