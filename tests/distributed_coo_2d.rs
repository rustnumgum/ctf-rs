...
fn leaf(
    ...
    a[0].default_coomm(2, &b[0], &1, &beta, &mut c[0]);
    c
}
fn contribution(step: usize, b: &[i64]) -> [i64; 4] {
    if step % 3 == 0 {
        [0; 4]
    } else {
...
fn run(context: &Context<'_>) {
    ...
    for step in 0..edge {
        for (old, value) in expected.iter_mut().zip(contribution(step, &b)) {
            *old += value;
        }
    }
    assert_eq!(c, vec![expected]);
    let moving_b: Vec<_> = [rank, rank + p]
        .into_iter()
        .map(|s| vec![s as i64, 2, 3, 4])
        .collect();
    let c = execute_coo_dense(
        &Arithmetic::<i64>::new(),
        edge,
    ...
    for moving in [false, true] {
        for outer in [1, 2] {
            ...
            let c = execute_coo_dense(
                ...
                &a,
                &[b.clone()],
                vec![vec![10; 4]; outer * local_steps],
                2,
                |a, b, mut c, beta, _| {
                    for (strip, block) in c.iter_mut().enumerate() {
                        a[0].default_coomm(2, &b[0], &(strip as i64 + 1), &beta, block);
                    }
                    c
                },
            );
            ...
            for strip in 0..outer {
                for local in 0..local_steps {
                    ...
                    let factor = (strip + 1) * if moving { p } else { 1 };
                    expected.push(
                        contribution(step, &b)
                            .into_iter()
                            .map(|v| 20 + factor as i64 * v)
                            .collect::<Vec<_>>(),
                    );
                }
            }
...
fn main() {
    ...
    if world.rank() == 0 {
        println!(
            "DIGIT / PASS distributed_coo_2d: native COO leaf, variable duplicate/zero panels, A/B broadcast, dense C cyclic reduction and strided scatter; exact i64; world+parity"
        );
    }
    world.close();
    drop(universe);
