...
fn models(scale:f64)->Models{
    ...
    for name in ["seq_tsr_ctr_mdl_ref","seq_tsr_ctr_mdl_cst","seq_tsr_ctr_mdl_inr","bcast_mdl","red_mdl","dgtog_res_mdl"]{
...
fn records(c:&Context<'_>,old:[&Distribution;3],m:&Models,limit:usize,folding:bool)->Vec<Record>{
    ...
    let mut add=|id,exhaustive,mapped:[Distribution;3]|{
        ...
        if memory<limit{local.push(Record{id,exhaustive,seconds,memory});}
    };
...
fn main(){let runtime=Runtime::initialize();let world=runtime.world();run(&world);
    ...
    if world.rank()==0{println!("DIGIT / PASS dense_search: folded/unfolded rank-order winners, weighted two-pass, exhaustive refinement, .01 cutoff, strict memory, custom no-fold and cache; world+parity; exact");}
